...
assert.equal(initialRun().energy,72);assert.equal(initialRun().energyCap,72);
assert.equal(resolveFloor(state(),()=>.9).energy,71);
const tourist=rider('tourist'),child=rider('child');
assert.equal(resolveFloor(state({cabin:[tourist,null,null,null,null,null]}),()=>.9).energy,69);
...
assert.equal(passengerEnergy(six),12);assert.equal(resolveFloor(six,()=>.9).energy,59);
...
assert.equal(resolveFloor(state({cabin:[rider('mechanic','m1'),rider('mechanic','m2'),rider('tourist','t1'),null,null,null]}),()=>.9).lastEnergy.delta,-1,'two mechanics stack to offset four points of passenger energy');
assert.equal(energySavings(state({cabin:[rider('ghost','g1'),rider('exorcist','e1'),rider('ghost','g2'),null,rider('tourist','t1'),null]})),2,'each controlled ghost contributes one stack of savings');
const doubleSupport=state({floor:1,cabin:[rider('celebrity','c1'),rider('tourist','t1',{destination:2}),rider('celebrity','c2'),null,null,null]});
assert.equal(resolveFloor(doubleSupport,()=>.9).lastEarnings.sources.find(x=>x.label==='游客到站')?.amount,28,'two green links each pay the cooperation bonus');
const doubleConflict=state({floor:1,cabin:[rider('inspector','i1'),rider('ghost','g1'),rider('inspector','i2'),null,null,null]});
assert.equal(riderAgitation(doubleConflict,1).low,2,'two red links each add agitation');
const protectedConflict=state({floor:1,cabin:[rider('inspector','i1'),rider('ghost','g1'),rider('exorcist','e1'),null,null,null]});
assert.equal(riderAgitation(protectedConflict,1).low,0,'one green link protects that rider from all neighbor conflicts');
const doubleCoach=state({floor:1,cabin:[rider('coach','c1'),rider('tourist','t1',{destination:2}),rider('coach','c2'),null,null,null]});
assert.equal(resolveFloor(doubleCoach,()=>.9).lastEarnings.sources.find(x=>x.label==='游客到站')?.amount,44,'each adjacent coach adds another fifty percent of base fare');
const tripleLover=state({floor:1,cabin:[rider('lover','l1'),rider('lover','l2',{destination:2}),rider('lover','l3'),null,null,null]});
const triplePaid=resolveFloor(tripleLover,()=>.9);
assert.equal(triplePaid.lastEarnings.sources.find(x=>x.label==='恋人连携')?.amount,4,'lover per-floor income stacks per lover link');
assert.equal(triplePaid.lastEarnings.sources.find(x=>x.label==='恋人到站')?.amount,24,'two lover neighbors make base fare triple and add two cooperation rewards');
...
assert.deepEqual(Array.from({length:10},(_,i)=>shiftAgitation(i+11,4)),[0,0,0,1,1,1,4,4,4,0]);
