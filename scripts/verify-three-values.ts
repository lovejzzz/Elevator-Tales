...
assert.deepEqual([INITIAL_ENERGY,ENERGY_CAPACITY,AGITATION_CAPACITY,HIGH_RISK_START,OFFER_PRESSURE_STEP,HIGH_RISK_BONUS],[48,60,6,30,40,8]);
const empty=state();assert.equal(resolveFloor(empty).floor,1);assert.match(resolveFloor(empty).message,/至少接一位/);
assert.equal(resolveFloor(state({cabin:[rider('tourist'),null,null,null,null,null]}),()=>.9).energy,45);
...
console.log(JSON.stringify({version:'v8.15',transitions,threeValues:true,deterministicAgitation:true,mandatoryRider:true}));
