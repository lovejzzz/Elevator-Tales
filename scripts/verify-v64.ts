...
import { CHARGE_PRICE, initialRun, shiftAgitation, shiftOutlook, resolveFloor, makeOffers, chargeBattery, chargingPlan, dismissalCost, dismissRider, type RunState, type Rider } from '../lib/game-engine';
...
for(let i=0;i<20000;i++){
 ...
 for(let sample=0;sample<2;sample++){
  const seed=970611+i*13+sample,encoded=JSON.stringify(before),after=resolveFloor(before,seeded(seed)),old=oldResolve(before,seeded(seed));
