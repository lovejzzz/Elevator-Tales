...
assert.deepEqual(chargingPlan(shop),{target:22,units:18,cost:18*CHARGE_PRICE,baseline:20});
const full=chargeBattery(shop,20);assert.equal(full.energy,24);assert.equal(full.coins,100-20*CHARGE_PRICE);assert.equal(full.earned,100);
