...
export function activeConnection(cabin: Array<Rider | null>, first: number, second: number): boolean {
  ...
  if (['nurse','musician'].includes(a.kind) || ['nurse','musician'].includes(b.kind)) return true;
...
export function planPlacement(state: RunState, candidate: Rider, target: number): PlacementResult {
  ...
  const message = combo && loverPair ? '恋人已配对：每位恋人邻座让本人到站基价 +100%；途中不产币。' : combo ? `${PASSENGERS[rider.kind].name}与邻座联动已生效。` : source >= 0 ? swapped !== state.swapped ? '站位已调整 · 本层旧乘客换位已用。' : '站位已调整 · 不消耗旧乘客换位。' : `${PASSENGERS[rider.kind].name}已站到 ${target + 1} 号位。`;
