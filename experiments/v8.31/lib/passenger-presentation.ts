import { MECHANIC_SAVING, PASSENGERS, passengerCategory, type PassengerKind } from './game-data';
import { bondLines, bondSummary, riderConflictRules, riderProfile, type ConflictEffect } from './rider-profile';
import { arrivalFare, HIGH_RISK_BONUS, INSPECTOR_COMPLIANCE_REWARD, INSPECTOR_ENERGY_LIMIT, riderAgitation, type Rider, unlockedAt, type RunState } from './game-engine';
import { RISK_PARTNERS } from './shift-rules';

export const SHARED_SAVING_RULE = '维修工和受控幽灵的节能逐项相加，最多抵完人物耗电；运转耗电随楼层增加。压力回收在到站舒缓后另算。';
...
export function passengerFace(rider: Rider, state: RunState) {
 ...
 switch(rider.kind){
  case 'tourist':moneyNote='每位邻座：到站+2币';special='包括其他游客；邻座变化即重算';break;
  case 'courier':special='到站补充2电（不超过上限）';break;
  case 'lover':moneyNote='每位邻座恋人：到站基价+100%';special='无恋人邻座：每站25%呼唤恋人';break;
  case 'thief':moneyNote='无警察/律师：每站+3；受控仅到站+5';pressure.splice(0,1,`每层 +1`,'挨警察或律师免除');break;
  case 'drunk':moneyNote='躁动≥3且2+邻座：基价+100%';pressure.splice(0,1,`每层 +1`,'挨护士或音乐家免除');break;
  ...
  case 'inspector':moneyNote=`人物净耗电≤${INSPECTOR_ENERGY_LIMIT}：每层+${INSPECTOR_COMPLIANCE_REWARD}币`;pressure.splice(0,1,`人物净耗电>${INSPECTOR_ENERGY_LIMIT}：每层 +1`);special='只检查人物与红线，含本人；扣除稳压和节能，不含运转';break;
...
export function passengerCardSections(
 ...
 switch(rider.kind){
  ...
  case 'lover':
   ...
   break;
  case 'tourist':addGreen(null,[effect('coins','到站 +2/人')],'任何邻座');break;
  ...
  case 'thief':
   ...
   addGreen(['cop','lawyer'],[effect('coins','途中不产币'),effect('coins','到站 +5'),effect('agitation','不加躁动')]);
   break;
  case 'cop':
  ...
  case 'drunk':
   ...
   self.push(effect('coins','关门躁动≥3且2+邻座：基价 +100%'));
   addGreen(['musician','nurse'],[effect('agitation','不加躁动')]);
   break;
  ...
  case 'celebrity':
   ...
   break;
  case 'inspector':
   self.push(effect('coins',`人物净耗电≤${INSPECTOR_ENERGY_LIMIT}：每上1层立即 +${INSPECTOR_COMPLIANCE_REWARD}`),effect('agitation',`人物净耗电>${INSPECTOR_ENERGY_LIMIT}：每上1层 +1躁动`));
   break;
  case 'bomb':
 ...
 const bondEffects = [effect('coins',`本人到站时 +${bonus}/人`),
 ...
 const risk: PassengerCardRelation[] = passengerCategory(rider.kind) === 'bad' ? [{ targets: RISK_PARTNERS.filter(kind => visible.has(kind)), effects: [effect('coins','未受控相邻：每人暂存 +2/层'), effect('agitation','每条链接 +1躁动/层'), effect('neutral','送达兑现；请离全部放弃')] }] : [];
...
export function passengerBrief(rider: Rider, floor: number, cabin: Array<Rider|null>=[], bonus=1, relief=0, multiplier=1, agitation=0) {
 ...
 const expectedFare=profile.hidden?null:slot<0?profile.fare+(rider.volatile?HIGH_RISK_BONUS:0)+rider.fareBonus:arrivalFare(rider,cabin,slot,bonus,agitation);
