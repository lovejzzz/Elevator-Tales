...
export function passengerFace(rider: Rider, state: RunState) {
 ...
 switch(rider.kind){
  ...
  case 'thief':moneyNote='无警察：每层偷邻座 1–4 币；受控仅到站+5';pressure.splice(0,1,`每层 +1`,'挨警察免除');break;
...
export function passengerCardSections(
 ...
 switch(rider.kind){
  ...
  case 'thief':
   self.push(effect('coins','未受控：每层偷邻座 1–4 币'),effect('agitation','未受控：每上1层 +1躁动'));
   addGreen(['cop','lawyer'],[effect('coins','不再偷钱'),effect('coins','到站 +5'),effect('agitation','全车 −1/层')]);
   break;
  case 'cop':
