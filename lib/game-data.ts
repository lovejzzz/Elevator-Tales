...
export const PASSENGERS: Record<PassengerKind, PassengerSpec> = {
  ...
  scandal: { kind:'scandal', name:'丑闻明星', title:'The Scandal', weight:1, fare:14, energy:1, trip:[4,7], patience:2, rarity:0, sheet:'03', cell:0, tone:'risk', short:'每位邻座每层+1币；偷拍客在旁再+3；检查员或黑警在旁：到站曝光，车费归零', detail:'暗黑版名人。邻座越多越好：每位邻座每层 +1 金币；挨着偷拍客每层再 +3。到站时如果挨着检查员或黑警，丑闻曝光：这一趟的车费和暂存全部归零。' },
  ...
  madbomber: { kind:'madbomber', name:'疯炸客', title:'The Mad Bomber', weight:1, fare:50, energy:1, trip:[3,5], patience:1, rarity:0, sheet:'03', cell:0, tone:'risk', short:'弗兰肯斯坦炸弹：倒计时更短，普通警察锁不住；归零时本班结束', detail:'暗黑版炸弹客，抱着一颗弗兰肯斯坦式的怪炸弹。实时倒计时比炸弹客短，普通警察锁不住，只有黑警挨着才能锁住，或用引线剪当场拆掉。归零时本班结束。车费 50。', risk: { label: '致命风险', guide: '与黑警相邻：锁住倒计时' } },
