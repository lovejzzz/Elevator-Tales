...
export const PASSENGERS: Record<PassengerKind, PassengerSpec> = {
  ...
  lover: { kind: 'lover', name: '恋人', title: 'The Lover', weight: 1, fare: 5, energy: 1, trip: [3, 7], patience: 4, rarity: 10, sheet: '01', cell: 4, tone: 'social', short: "每位邻座恋人：到站基价+100%", detail: "有未配对恋人时，每层全车统一25%概率呼唤1位恋人，不按人数叠加。成功时另两位为非恋人，仍可互相作用；未触发时按普通候客规则生成。每位相邻恋人让本人到站基价增加100%，线性叠加；途中不产金币。小费、默契和高危加价不参与倍率。" },
  ...
  ghost: { kind: 'ghost', name: '幽灵', title: 'The Apparition', weight: 0, fare: 3, energy: 0, trip: [4, 9], patience: 5, rarity: 6, sheet: '03', cell: 0, tone: 'occult', short: '0耗电；未受控到3的倍数层，随机1位邻座延误1站', detail: '相邻驱魔师时，不再延误邻座，每位受控幽灵每层抵消1点人物耗电且到站多得2金币；否则到3的倍数层时随机延误一位邻座1站。受控幽灵的节能逐项相加，但不能抵消电梯运转耗电。' },
  ...
  inspector: { kind: 'inspector', name: '检查员', title: 'The Inspector', weight: 2, fare: 8, energy: 1, trip: [4, 7], patience: 4, rarity: 5, sheet: '03', cell: 4, tone: 'support', short: '连续低躁动2层：到站额外+12币', detail: '连续两次关门时处于低躁动，获得合规印章，到站额外+12金币，不参与倍率。完成前离开低档会重新计数；完成后印章保留，每位检查员只奖励一次。不再检查人物耗电，也不因此增加躁动。' },
  ...
  don: { kind:'don', name:'教父', title:'The Don', weight:0, fare:0, energy:0, trip:[9,9], patience:9, rarity:0, sheet:'04', cell:0, tone:'risk', short:'传奇 · 每层暂存6币；每层+1躁动', detail:'第1层上车、第10层下车，不耗电。在车时每层暂存6金币，送达兑现；每层+1躁动，护士无法抵消。与他相邻的小偷照常赚途中收入但不加躁动。信物「怀表」：坏人链接每层多存1金币，坏人链接躁动每层最多1点。', risk:{label:'风险交易',guide:'用到站舒缓和护士长压住躁动'} },
  ...
  nightingale: { kind:'nightingale', name:'夜莺', title:'The Nightingale', weight:0, fare:0, energy:0, trip:[9,9], patience:9, rarity:0, sheet:'04', cell:0, tone:'social', short:'传奇 · 中躁动每层+6币', detail:'爵士歌手。第1层上车、第10层下车，不耗电、不付车费。关门时中躁动每层+6金币；低躁动时每层+1躁动。相邻游客多算1位邻座。信物「黑胶唱片」：音乐家从第1层起出现，中躁动时音乐家演出收入再+2、每位到站乘客小费再+2。' },
...
export const UPGRADES: Record<UpgradeKey, { name: string; label: string; description: string; strategy: string; tone: 'sustain' | 'control' | 'score' | 'capacity' | 'tempo' }> = {
  ...
  soundproof: { name: '隔音门', label: 'SOUNDPROOF DOOR', description: '每层抵消最多1点普通红线躁动；坏人链接不再产生躁动。不降低已有躁动，不抵消人物自身或高危躁动。', strategy: '容忍冲突', tone: 'control' },
