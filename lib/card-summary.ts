...
export function cardLine(rider: Rider, run: RunState, locale: GameLocale): { line: string; progress?: string; sealed?: boolean } {
  ...
  switch (rider.kind) {
    ...
    case 'otherthirteen': return { line: L(`每层随机一种：+${DARK_LEGEND_RULES.thirteenCoins}币、+${DARK_LEGEND_RULES.thirteenAgitation}躁动、−${DARK_LEGEND_RULES.thirteenPower}电或无事`, `Each floor, one at random: +${DARK_LEGEND_RULES.thirteenCoins} coins, +${DARK_LEGEND_RULES.thirteenAgitation} agitation, −${DARK_LEGEND_RULES.thirteenPower} power or nothing`) }; // v9.20.7 (English playtest 28): the odds space, not just “something good or bad”
    // v9.19 dark versions.
