...
export const V9211_ZH: ChangelogEntry = {
  version: '9.21.1', date: '2026-09-25', title: '夜市货：夜市卖只在夜市有的贵东西，每次都不一样',
  summary: '夜市以前卖的就是商店那几样道具。现在摊位上有两件“夜市货”——只在夜市卖、比商店道具更强也更贵——外加一件商店道具。四种夜市货每次随机出两种，所以每局的夜市都不一样。',
  changes: [
    '四种夜市货（商店里永远不卖）。价格按深度涨，和商店道具一样乘以（1 +（楼层 − 60）/ 200），81–89 层的价格如下：',
    '· 长明照明弹 210–218 币：本层和下一层所有暗黑版的麻烦都不发生（照明弹只管一层，136 币左右）。',
    '· 檀香 133–137 币：立即 −5 躁动（香薰是 −2）。',
    '· 强效镇静剂 88–92 币：一位乘客直到下车都不产生躁动、不会深渊发作、不会戒断（镇静剂只管 3 层）。',
    '· 大护身符 99–103 币：车里所有普通人这一趟都不会被同化（护身符只护一位）。',
    '夜市的摊位：随机两件夜市货，加一件 60 层以后的商店道具（照明弹、镇静剂、引线剪等，随机一件）。以前是三件商店道具，照明弹几乎每次都在。',
    '摊位改成一行一件：名字、“夜市货”标记、价格，下面写着效果。手机上没有悬停提示，以前只能看到名字和价格。',
    '夜市货用原道具的画，换成夜色调、加金边，一眼能和商店版分开。',
    '长明照明弹生效时，照明弹用不了（不会白白浪费）。',
    '说明书和夜市的规则说明都写了夜市货。',
  ],
  experiments: [
    '80–88 层逐层统计，200 局 × 3 种机器人。均衡型中位 88 层，到 90 层 37%，到 100 层 1.0%（200 局中 2 局；上一版 1 局）；合作型 100 层 0%；真人型中位 75 层。“这一层什么都没发生”：均衡型 13%，合作型 7%，和上一版差不多。',
    '机器人在夜市买什么（买完要剩 ≥40 币才买，优先长明照明弹，其次照明弹、强效镇静剂、镇静剂、檀香）。均衡型 / 合作型 / 真人型：强效镇静剂 22/60、26/48、8/14 次；长明照明弹 6/56、14/72、2/21 次；檀香 5/53、13/53、1/11 次。大护身符机器人不会用，所以不买。买完平均剩 93–111 币。长明照明弹 210 多币，大多数时候买不起，是真正的贵货。',
    '模拟器验收 4200 局：熟练型死因 断电 52.8% / 躁动 46.3% / 炸弹 0.9%；抵达 100 层 0.4%（上一版 0.6%）；均衡型中位 87 层（p10–p90 80–94）；真人型 78 层；新手 34 层；均衡型上车率全部在 16–61%。“不宽裕”两项仍不通过（66.4%、离店中位 28 币），和以前一样。',
    '浏览器试玩。中文窄屏，85 层夜市：花 214 币买长明照明弹、90 币买强效镇静剂，余额 420→116；给药贩用强效镇静剂，座位显示“还剩 7 层不躁动”（他 92 层下车）。点燃长明照明弹后，85 和 86 层发作几率都是 0，87 层回到 20%，药贩的镇静 7→5。英文手机：买了檀香和大护身符，躁动 7→2；车里只有暗黑版时大护身符是灰的。界面没有中文，页面没有横向滚动。',
    'verify 新增 v9.21.1 一组（v9 检查从 43 项变成 44 项，全部通过）。内容：200 次抽取里每次都是两件夜市货加一件商店道具，且三件不同；四种夜市货都会出现；商店从不卖夜市货；长明照明弹管两层；檀香 −5；强效镇静剂管到下车；大护身符只护普通人；英文翻译。',
  ],
  watch: [
    '大护身符只在车里有普通人、又怕被同化时有用，深渊里普通人不多，可能很少有人买。',
    '长明照明弹在 90 层以后（发作 30–50%）最值钱，但夜市在 81–89 层，只能留到最后两层前后用；看真人会不会囤着它过 90 层。',
    '夜市货没有配专门的画，用的是原道具的画换色。',
  ],
};

export const V9211_EN: ChangelogEntry = {
  version: '9.21.1', date: '2026-09-25', title: 'Night goods: the Night market sells pricey things found nowhere else, different every time',
  summary: 'The Night market used to sell the same items as the shops. Now its stall holds two “night goods” and one shop item. Night goods are sold only at the market and are stronger and dearer than shop items. Two of the four night goods turn up at random, so the market differs from run to run.',
  changes: [
    'Four night goods, never sold in shops. Like shop items, their prices rise with depth, ×(1 + (floor − 60) / 200). At 81–89F they cost:',
    '• Long Flare, 210–218 coins: no dark rider causes trouble this floor or the next. A Flare covers one floor, for about 136.',
    '• Sandalwood, 133–137 coins: −5 agitation now. Incense gives −2.',
    '• Strong Sedative, 88–92 coins: a rider adds no agitation of their own until they get off, with no abyss outbursts and no withdrawal. A Sedative lasts 3 floors.',
    '• Great Amulet, 99–103 coins: every normal rider aboard is safe from corruption this trip. An Amulet protects one rider.',
    'The stall holds two random night goods plus one random shop item from 60F on (Flare, Sedative, Wire Cutter and so on). It used to hold three shop items, nearly always including a Flare.',
    'The stall lists one item per row: name, a “Night goods” tag, price, and the effect below. Phones have no hover tooltips, so before this only the name and price showed there.',
    'Night goods reuse their shop item’s painting in night colors with a gold edge, so they are easy to tell from the shop versions.',
    'A Flare cannot be lit while a Long Flare is in effect, so it is never wasted.',
    'The manual and the market’s rule line describe the night goods.',
  ],
  experiments: [
    'Floor-by-floor stats for 80–88F, 200 runs × 3 bots:',
    '• Balanced bot: median 88F; 90F reached in 37% of runs, 100F in 1.0% (2 runs of 200; the last release had 1).',
    '• Cooperation bot: 100F reached in 0%.',
    '• Human-like bot: median 75F.',
    '• Floors where nothing happened: 13% for the balanced bot and 7% for the cooperation bot, close to the last release.',
    'What the bots buy at the market. They buy only if at least 40 coins are left afterwards, trying the Long Flare first, then the Flare, Strong Sedative, Sedative and Sandalwood. Purchases per offer (balanced / cooperation / human-like):',
    '• Strong Sedative: 22/60, 26/48, 8/14.',
    '• Long Flare: 6/56, 14/72, 2/21.',
    '• Sandalwood: 5/53, 13/53, 1/11.',
    '• Great Amulet: never, because the bots have no use for it.',
    '• 93–111 coins are left on average after buying. At over 210 coins the Long Flare is usually out of reach, so it is a real luxury.',
    'Acceptance, 4,200 runs:',
    '• Skilled endings: 52.8% power, 46.3% agitation, 0.9% bomb.',
    '• 100F reached in 0.4% of runs (0.6% in the last release).',
    '• Balanced median 87F (p10–p90 80–94); human-like 78F; novice 34F.',
    '• Every balanced boarding rate is within 16–61%.',
    '• Both “not affluent” checks still fail, as before (66.4%; median 28 coins left after a shop).',
    'Browser playtest, Chinese, narrow window, 85F market:',
    '• I bought a Long Flare for 214 coins and a Strong Sedative for 90; coins went from 420 to 116.',
    '• I sedated the Dealer, who gets off at 92F; his seat read “no agitation for 7 more floors”.',
    '• After I lit the Long Flare, the outburst odds were 0 on 85F and 86F and back to 20% on 87F. The Dealer’s sedation went 7→5.',
    'Browser playtest, English, phone:',
    '• Sandalwood took agitation from 7 to 2.',
    '• The Great Amulet stayed greyed out with only dark riders aboard.',
    '• No Chinese appeared in the interface, and the page did not scroll sideways.',
    'verify: a new v9.21.1 group raises the v9 checks from 43 to 44, all passing. It tests that:',
    '• in 200 draws, every stall holds two night goods and one shop item, three different items in all;',
    '• all four night goods turn up, and shops never sell them;',
    '• the Long Flare covers two floors, Sandalwood gives −5, the Strong Sedative lasts until the rider gets off, and the Great Amulet protects only normal riders;',
    '• the English translations are correct.',
  ],
  watch: [
    'The Great Amulet helps only when normal riders aboard risk corruption. There are few normal riders in the abyss, so it may rarely be bought.',
    'The Long Flare is worth most past 90F, where outburst odds run 30–50%. The market is at 81–89F, though, so it can only be carried there. Watch whether players save it for the 90s.',
