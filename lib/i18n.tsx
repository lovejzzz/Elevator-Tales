...
const exactPairs: Array<[string, string]> = [
  ...
  ['单独时：15% 呼唤恋人', 'Alone: 15% chance to call a Lover'],
  ...
  ['初始50电、容量60。抵达商店先补 5 电，再用金币充电或买升级。急躁乘客多赚 8 金币，但每层多 +1 躁动。', 'Start with 50 power and a capacity of 60. Entering the Shop restores 5 power; then spend coins on charging or upgrades. High Risk riders earn 8 extra coins but add +1 agitation per floor.'],
  ...
  ['无恋人邻座：每站15%呼唤恋人', 'No adjacent Lover: 15% chance per floor to call another Lover'],
  ...
  ['没有恋人邻座：每层有 15% 概率呼唤另一位恋人候客。', 'With no adjacent Lover: 15% chance each floor to call another Lover into the next queue.'],
  ...
  ['独处时，每层有15%概率让下一批候选出现另一位恋人。回应者会带有专属标记。每位相邻恋人都让本人每层多赚1金币、到站基础车费增加100%；多条恋人连接线性叠加，小费不参与倍率。', 'When alone, there is a 15% chance each floor that another Lover joins the next candidates. Responders carry a special mark. Each adjacent Lover grants +1 coin per floor and +100% base arrival fare; multiple links stack linearly and tips are not multiplied.'],
  ...
  ['人物卡明确显示每层 0、+1 或 +2；急躁乘客固定再 +1。', 'Rider cards explicitly show 0, +1, or +2 per floor. High Risk adds another +1.'],
...
function translateUncached(value: string): string {
  ...
  let translated = value
    ...
    .replace(/(无人认领的纸箱开箱|小孩拆开纸箱|复制人打开复制箱)：(?:(\d+) 金币|(\d+) 电的电池|能力「(.+?)」)/gu, (_m, who: string, coins?: string, power?: string, ability?: string) => `${({ '无人认领的纸箱开箱': 'The unclaimed box held', '小孩拆开纸箱': 'The Child opened a box:', '复制人打开复制箱': 'The Mimic opened a copied box:' } as Record<string, string>)[who]} ${coins ? `${coins} coins` : power ? `a ${power}-power battery` : `the ability “${ability}”`}`)
    .replace(/（安装位已满，可以替换一项）/gu, ' (every slot is full: you can swap one out)')
    .replace(/^恋人配对 ×(\d+)$/u, 'Lover pairing ×$1')
    .replace(/^能力「(.+)」$/u, 'Ability “$1”')
    ...
    .replace(/^(-?\d+) 金币$/u, '$1 coins')
    .replace(/^每条默契到站加成 \+(\d+) 金币$/u, 'Each bond: +$1 coins on arrival')
    .replace(/^躁动上限 (\d+) · 手动调节可用（−3 躁动）$/u, 'Agitation cap $1 · manual calming ready (−3 agitation)')
    .replace(/^躁动上限 (\d+) · 手动调节已用，下个商店补满$/u, 'Agitation cap $1 · manual calming used, refilled at the next shop')
    .replace(/^放弃了纸箱里的能力「(.+)」。$/u, 'Passed on the box’s ability “$1”.')
    .replace(/^装上「(.+?)」，换下的「(.+?)」会在下次进商店时卖掉（退 (\d+) 金币）。$/u, 'Installed “$1”; “$2” is sold at the next shop ($3 coins back).')
    .replace(/小孩拆开纸箱：(\d+) 金币/gu, 'The Child opened a box: $1 coins')
    .replace(/小孩拆开纸箱：(\d+) 电的电池/gu, 'The Child opened a box: a $1-power battery')
    .replace(/复制人打开复制箱：(\d+) 金币/gu, 'The Mimic opened a copied box: $1 coins')
    .replace(/复制人打开复制箱：(\d+) 电/gu, 'The Mimic opened a copied box: $1 power')
    .replace(/检查员验货：快递员晚一层到站，签收多付 (\d+) 金币/gu, 'Inspector checked the box: the Courier arrives a floor later and pays $1 coins more')
    .replace(/维修工拆了纸箱当零件：检修完成，后续(\d+)层运转少耗1电/gu, 'The Mechanic used a box for parts: repair done, motor −1 power for the next $1 floors')
    .replace(/顺手牵羊 \+(\d+)\/层/gu, 'Picking pockets +$1/floor')
    .replace(/无人认领 · 到站开箱 (\d+) 币或 (\d+) 电/gu, 'Unclaimed · opens for $1 coins or $2 power')
    ...
    .replace(/连续不高躁动(\d+)层：到站 \+(\d+)金币/gu, '$1 floors in a row below high agitation: +$2 arrival coins')
    ...
    .replace(/^热闹和江湖每级绿线每层 \+(\d+) 金币$/u, 'Lively and Street: +$1 coins per green-link level per floor')
    .replace(/^热闹和江湖每级绿线每层 \+(\d+) → \+(\d+) 金币。$/u, 'Lively and Street: +$1 → +$2 coins per green-link level per floor.')
    ...
    .replace(/^两位(.+?)单看不划算，挨着坐就赚：一起带上。$/u, (_m, a: string) => `Two ${translateGameText(a, 'en')}s do little on their own but earn side by side: take both.`)
    .replace(/^(.+?)单看不划算，和(.+?)挨着坐就赚：一起带上。$/u, (_m, a: string, b: string) => `The ${translateGameText(a, 'en')} does little on its own but earns beside the ${translateGameText(b, 'en')}: take them together.`)
    ...
    .replace(/^商店舒缓 −(\d+) 躁动，支付 (\d+) 金币。$/u, 'Shop calming −$1 agitation for $2 coins.')
    .replace(/^重抽能力，支付 (\d+) 金币。$/u, 'Rerolled the abilities for $1 coins.')
    .replace(/^「(.+)」升到 2 级，支付 (\d+) 金币。$/u, (_m, name: string, n: string) => `${translateGameText(name, 'en')} raised to level 2 for ${n} coins.`)
    .replace(/^买下(.+)，支付 (\d+) 金币。$/u, (_m, item: string, n: string) => `Bought ${translateGameText(item, 'en')} for ${n} coins.`)
