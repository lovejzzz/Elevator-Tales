...
import { DARK_RULES, ITEMS, ITEM_SLOTS, MYSTERY_RULES, abyssTier, corruptible, isBombKind, isCarrierKind, isSurvivor, type ItemKey } from '@/lib/dark-rules';
...
export default function ElevatorGame() {
  ...
  useEffect(() => {
    ...
    if (run.status === 'upgrade') { seenEvents.current = { blast: run.lastBlast, corruption: run.lastCorruption, summons: run.lastSummons }; return; }
  ...
  }, [run.lastBlast, run.lastCorruption, run.lastSummons, run.cabin, run.status, language]);
