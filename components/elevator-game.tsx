...
import { agitationBand, musicBeatForAgitation, motorAdvanceNotice, motorScheduleText, nightUnrest, nightUnrestText, REPAIR_WORK, INSPECTION_WORK, INSPECTION_BONUS, CHILD_CARE_WORK, RESERVE_CELL_CHARGE } from '@/lib/balance-v832';
...
import { DARK_RULES, ITEMS, ITEM_SLOTS, MYSTERY_CLUES, MYSTERY_RULES, abyssStep, abyssTier, outburstChance, outburstIsPower, mysteryClue, corruptible, isBombKind, isCarrierKind, isSurvivor, type ItemKey } from '@/lib/dark-rules';
...
/** v9.20.1: the ascend button asks for a second press when an abyss floor has at least this chance to boil over. */
const GAMBLE_WARN = 0.05;
...
const DARK_LEGEND_REWARD: Record<DarkLegendKind, [string, string]> = {
  ...
  highroller: [`低躁动 +${DARK_LEGEND_RULES.highRollerWin} 金币，否则 −${DARK_LEGEND_RULES.highRollerLoss}`, `+${DARK_LEGEND_RULES.highRollerWin} coins if calm, otherwise −${DARK_LEGEND_RULES.highRollerLoss}`],
...
export function PassengerCardFace({ rider, run, action, locale }: { rider: Rider; run: RunState; action?: string; locale: GameLocale }) {
  ...
  return <span className="unified-passenger-summary compact-card" data-no-translate>
    <span className="cc-head"><Portrait kind={rider.kind} rider={rider}/><span className="cc-title"><strong>{displayName(rider,locale)}<span className={`card-gem gem-${riderCardGrade(rider)}`} title={({standard:zh?'普通':'Common',fine:zh?'精良':'Fine',rare:zh?'稀有':'Rare',legendary:zh?'传奇':'Legendary'} as Record<string,string>)[riderCardGrade(rider)]} aria-hidden="true" /></strong><span className="cc-sub">
      ...
      {isDark(rider.kind)&&outburstChance(run.floor+1)>0&&<span className="cc-tag cc-tag-outburst" title={zh?`深渊里的暗黑版越来越极端：每层有 ${Math.round(outburstChance(run.floor+1)*100)}% 的机会发作（${outburstIsPower(rider.kind)?`吸走 ${DARK_RULES.outburstPower} 电`:`+${DARK_RULES.outburstAgitation} 躁动`}）；深渊里上车的暗黑版车费更高。照明弹、镇静剂能压住。`:`Deep in the abyss dark riders grow extreme: each floor a ${Math.round(outburstChance(run.floor+1)*100)}% chance to lash out (${outburstIsPower(rider.kind)?`draining ${DARK_RULES.outburstPower} power`:`+${DARK_RULES.outburstAgitation} agitation`}); dark cards drawn in the abyss pay more. A Flare or a Sedative holds it off.`}>{zh?`发作 ${Math.round(outburstChance(run.floor+1)*100)}% · ${outburstIsPower(rider.kind)?`−${DARK_RULES.outburstPower}电`:`+${DARK_RULES.outburstAgitation}躁`}`:`Lashes out ${Math.round(outburstChance(run.floor+1)*100)}% · ${outburstIsPower(rider.kind)?`−${DARK_RULES.outburstPower} power`:`+${DARK_RULES.outburstAgitation} agit.`}`}</span>}
    ...
    {legend ? <span className="cc-values cc-legend-values"><span>{rider.kind==='coldmatron'?(zh?'不付车费 · 药品耗电':'No fare · her drugs use power'):(zh?'不付车费 · 不耗电':'No fare · no power')}</span></span> : <span className="cc-values">
...
function riderState(cabin: Array<Rider | null>, slot: number, bonus: number, agitation: number, floor = 0): { label: string; tone: 'active' | 'warn' | 'neutral' } | null {
  ...
  switch (rider.kind) {
    ...
    case 'highroller': return agitationBand(agitation) === 'low' ? { label: `押注中 · 安静到站 +${DARK_LEGEND_RULES.highRollerWin}币`, tone: 'active' } : { label: `要输了 · 现在到站 −${DARK_LEGEND_RULES.highRollerLoss}币`, tone: 'warn' };
...
const PRESSURE_RELIEF: Array<[string, string, string, string]> = [
  ...
  ['护士与音乐家', '护士抵消每位邻座自身的躁动（每人每层最多 1 点），挡不住夜深人躁；音乐家把躁动拉向中档，中档时每层 +2 金币。', 'Nurse and Musician', 'A Nurse cancels up to 1 of each neighbor’s own agitation per floor but not late-night unrest; a Musician pulls agitation toward medium and earns 2 per floor there.'],
...
export default function ElevatorGame() {
  ...
  const emergencyNeed = run.status === 'playing' ? Math.min(emergencyLeft, Math.max(0, 1 - sector.projected, risk.need)) : 0;
  // v9.20.1: “fatal” means certain to be possible without abyss outbursts; outbursts are a priced gamble.
  const stressFatal = run.stress + (pressurePreview.certainHighDelta ?? pressurePreview.highDelta) >= run.stressCap;
  const gambleChance = stressFatal ? 0 : pressurePreview.lossChance ?? 0;
  const gamble = run.status === 'playing' && gambleChance >= GAMBLE_WARN;
  const gambleCalm = gamble ? Math.max(1, run.stress + pressurePreview.highDelta - run.stressCap + 1) : 0;
  // v9.6 in-transit calming: coins lower agitation before departure, capped per ten floors.
  const calmLeft = calmAllowance(run);
  const calmNeed = stressFatal ? Math.max(1, run.stress + (pressurePreview.certainHighDelta ?? pressurePreview.highDelta) - run.stressCap + 1) : 0;
  ...
  const content = <main className={`game-shell ${cooperationRelief(run) ? 'has-contract' : ''} ${difficultyTier(run.floor) % 2 ? 'phase-dawn' : ''}`}>
    ...
    <section className="game-grid">
      <aside className="status-rail">
        ...
        <div data-metric="energy" className={`meter-card energy ${energyFatal ? 'meter-danger' : ''}`} title={`${energyPreview.summary} · ${language==='zh'?`运转 ${travelEnergyCost(run.floor+1)} 电/层`:`motor ${travelEnergyCost(run.floor+1)}/floor`}${nextIsShop?'':` · ${motorAdvanceNotice(run.floor)}`}`}><div><BatteryCharging aria-hidden="true" /><span className="rail-metric-name">电量</span><b><AnimatedNumber value={run.energy} /><span className="metric-cap">/{run.energyCap}</span></b></div><MetricResponse metric="energy" event={metricEvent} locale={language} /><PowerGauge value={run.energy} cap={run.energyCap} next={run.energy + energyPreview.lowDelta} danger={Math.max(1, -energyPreview.lowDelta)} locale={language} /><small className="rail-forecast rail-power-line" data-no-translate><span>{language==='zh'?'下一站':'Next'} <b className={energyFatal ? 'forecast-fatal' : ''}>{energyPreview.range}</b></span>{run.status==='playing'&&<span className={`sector-forecast ${sector.failFloor!==null?'is-danger':sector.projected<8?'is-warn':''}`}>{language==='zh'?(sector.failFloor!==null?`${sector.failFloor} 层断电`:`到店剩 ${sector.projected}`):(sector.failFloor!==null?`out at ${sector.failFloor}F`:`${sector.projected} at shop`)}</span>}</small>
          {run.status==='playing'&&(sector.failFloor!==null||sector.projected<12||energyFatal)&&<div className="emergency-charge" data-no-translate title={language==='zh'?`本段还可补 ${emergencySectorLeft(run)} 电 · 途中价 ${emergencyPrice} 币/电`:`${emergencySectorLeft(run)} left this sector · ${emergencyPrice} coins each`}>
            {(()=>{const units=emergencyNeed>0?emergencyNeed:1;return <button className={emergencyNeed>0?'is-urgent':''} disabled={locked||emergencyLeft<units||run.coins<units*emergencyPrice} onClick={()=>emergency(units)}>{language==='zh'?`补电 +${units} · ${units*emergencyPrice}币`:`Charge +${units} · ${units*emergencyPrice}c`}</button>;})()}
          </div>}
</div>
        <div data-metric="stress" className={`meter-card pressure ${agitated || pressurePreview.tone === 'danger' ? 'meter-danger' : ''}`} title={pressurePreview.summary}>
          ...
          <small className="rail-forecast"><span>下一站 <b className={stressFatal ? 'forecast-fatal' : ''}>{pressurePreview.range}</b></span>{Math.round(gambleChance*100)>0&&<span className="rail-gamble" data-no-translate>{language==='zh'?`失控 ${Math.round(gambleChance*100)}%`:`Boil-over ${Math.round(gambleChance*100)}%`}</span>}</small>
      ...
      <aside className="arrival-panel">
        ...
        <div className={`candidate-panel ${draggedReturnable ? 'drop-return' : ''}`} onDragOver={(event)=>{ if (draggedReturnable) { event.preventDefault(); event.dataTransfer.dropEffect = 'move'; } }} onDrop={dropOnOffers} data-return-label={language==='zh'?'拖回这里撤回':'Drop here to withdraw'}><div className={`passenger-list ${offers.length > 3 ? 'has-four' : ''} ${!intro && !fastReveal && run.status === 'playing' ? 'offer-revealing' : ''}`} key={run.floor} role="list" aria-label="本层候客乘客">
        ...
        <div className="departure-controls">
          ...
          {run.status==='playing'&&doors==='open'&&strandedCourier&&!stressFatal&&!risk.fatal&&<div className={`power-alert ${departArmed?'is-armed':''}`} role="alert" data-no-translate>
            <p><Package aria-hidden="true"/>{language==='zh'?'快递员的纸箱还没上车':'The Courier’s box is not aboard'}</p>
            <small>{language==='zh'?'没有纸箱，他每层 +1 躁动，到站也不付钱。把纸箱放在他上下左右任一格；再按一次上行＝不带纸箱出发。':'Without it he adds +1 agitation a floor and pays nothing. Seat the box right beside him; press ascend again to leave without it.'}</small>
          </div>}
          {run.status==='playing'&&doors==='open'&&occupied>0&&gamble&&!risk.fatal&&<div className={`power-alert is-gamble ${departArmed?'is-armed':''}`} role="alert" data-no-translate>
            ...
            {(()=>{const slots=outburstSlots(run),drains=slots.filter(i=>outburstIsPower(run.cabin[i]!.kind)).length,rage=slots.length-drains,p=Math.round(outburstChance(run.floor+1)*100);return <small>{language==='zh'?`${slots.length} 位暗黑版可能发作（各 ${p}%）：${[rage?`${rage} 位 +${DARK_RULES.outburstAgitation}躁动`:'',drains?`${drains} 位吸电 −${DARK_RULES.outburstPower}`:''].filter(Boolean).join('，')}。请离几位、点照明弹或打镇静剂都能降低风险。`:`${slots.length} dark riders may lash out (${p}% each): ${[rage?`${rage} for +${DARK_RULES.outburstAgitation} agitation`:'',drains?`${drains} draining ${DARK_RULES.outburstPower} power`:''].filter(Boolean).join(', ')}. Dismissing some, a Flare or a Sedative lowers the risk.`}</small>;})()}
    ...
    <Dialog open={run.status === 'upgrade' && doors === 'open'}><DialogContent className={`story-dialog upgrade-dialog ${upgradeCrisis ? 'upgrade-crisis' : ''}`} showCloseButton={false}>
      ...
      {upgradeCrisis && <p className="shop-warning">{upgradeCrisis === 'both' ? '电量与躁动同时失控：底部最低抢救可恢复1电，并将躁动降至上限以下1点。' : upgradeCrisis === 'energy' ? '电量已耗尽：使用下方充电服务，将电量恢复到 0 以上才能继续。' : '躁动失控：底部最低抢救每点8金币，只降到上限以下1点；想再降，用下方的“安抚”（正常价格，计入本段额度）。'}{!emergencyRepairPlan(run).affordable&&upgradeCount>0&&run.coins+SELL_REFUND>=emergencyRepairPlan(run).cost?` 金币不够抢救：卖掉一项能力（退 ${SELL_REFUND} 币）就付得起。`:''}{upgradeCrisis!=='energy'&&!run.upgrades.calm&&run.shop.some(card=>card.key==='calm'&&!card.purchased)?(upgradeCount>=UPGRADE_SLOTS?' 也可以卖掉一项能力、换上“安全余量”：躁动上限 +2，再用它的手动调节 −3。':' 也可以免费装上“安全余量”：躁动上限 +2，再用它的手动调节 −3。'):''} 若无力修复，本班将在这里结束。</p>}
