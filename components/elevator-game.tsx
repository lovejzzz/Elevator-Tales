...
import { EMPTY_UPGRADES, failureLesson, hasNeighbour, initialRun, installUpgrade, isFreeReseat, makeOffers, neighbourCount, neighbours, readyPartner, resolveFloor, synergyPartnerAtSlot, totalWeight, unlockedAt, upgradeChoices, type Rider, type RunState, type UpgradeCrisis } from '@/lib/game-engine';
...
export default function ElevatorGame() {
  ...
  const weight = useMemo(() => totalWeight(run.cabin), [run.cabin]); const cabinFull = run.cabin.every(Boolean); const unlocked = unlockedAt(Math.max(run.floor, highest));
  ...
  return <main className={`game-shell ${run.floor >= 50 ? 'phase-dawn' : ''} ${run.status === 'won' ? 'shift-won' : ''}`}>
    ...
    <section className="game-grid">
      ...
      <aside className="arrival-panel">
        ...
        <div className="passenger-list">{offers.map((offer) => { const spec = PASSENGERS[offer.kind]; const boarded = run.cabin.some((rider) => rider?.id === offer.id); const pending = pendingOfferId === offer.id; const full = !boarded && cabinFull; const tooHeavy = !boarded && weight + spec.weight > run.weightCap; const unavailable = full || tooHeavy; const isDragging = dragged?.type === 'offer' && dragged.id === offer.id; const choosing = !boarded && !pending && !unavailable; const partner = unavailable ? null : readyPartner(offer.kind, run.cabin, offer.id); return <button className={`passenger-card tone-${spec.tone} ${offer.calledByLover ? 'lover-called' : ''} ${firstPairLesson && offer.kind === 'lover' ? 'guided-lover' : ''} ${boarded ? 'boarded' : ''} ${pending ? 'pending' : ''} ${isDragging ? 'dragging' : ''}`} key={offer.id} onClick={() => toggleOffer(offer)} draggable={!locked && !unavailable} onDragStart={(event) => startDrag(event, { type: 'offer', id: offer.id })} onDragEnd={endDrag} disabled={locked || unavailable} aria-pressed={boarded || pending} title={full ? '轿厢已满，等待乘客到站后再接人。' : tooHeavy ? `载重会超过 ${run.weightCap}，暂时无法上车。` : spec.detail}><Portrait kind={offer.kind} /><span className="passenger-copy"><strong>{spec.name}</strong><small>{spec.title} · 前往 {offer.destination}F</small><span className="tag-row"><i>{spec.weight} 载重</i><i>{offer.patience} 耐心</i>{partner ? <i className="ready-tag">可联动 · {PASSENGERS[partner].name}</i> : firstPairLesson && offer.kind === 'lover' ? <i className="lesson-tag">教学配对</i> : offer.calledByLover ? <i className="call-tag">回应呼唤</i> : null}{offer.fuse !== undefined && <i>引信 {offer.fuse}</i>}</span><em>{spec.short}</em>{spec.risk && <span className={`risk-guide ${spec.risk.label === '致命风险' ? 'fatal' : ''}`}><b>{spec.risk.label}</b><span>{spec.risk.guide}</span></span>}</span><span className={`select-mark ${choosing ? 'reward-mark' : ''}`}>{boarded ? '✓' : pending ? '→' : full ? '满' : tooHeavy ? '×' : <><b>{spec.fare + offer.fareBonus}</b><small>底价</small><em>+{spec.energy}能</em></>}</span></button>; })}</div>
    ...
    <footer className="footer-line"><span>ELV–07 / v4.5</span><i /><span>THE CITY NEVER REALLY SLEEPS</span></footer>
    ...
    <Dialog open={pressureHelp} onOpenChange={setPressureHelp}><DialogContent className="story-dialog pressure-dialog"><p className="dialog-kicker">PRESSURE CONTROL</p><DialogHeader><DialogTitle>压力从哪里来？</DialogTitle><DialogDescription>普通乘坐和电梯耗能都不会加压。压力达到 {run.stressCap} 时本局结束；每次关门前，系统会按当前站位预报下一层，随机事件与幽灵延误会显示为变化范围。</DialogDescription></DialogHeader><div className="pressure-rule-grid"><section className="pressure-rise"><small>会增加压力</small><b>耐心归零</b><p>每位离开的乘客 +2</p><b>未受控制的小偷</b><p>每逢偶数层 +1</p><b>未被安抚的醉汉</b><p>每层 25% 概率 +2</p><b>被多人围住的名人</b><p>两名以上邻座时，偶数层 +1</p><b>检查员发现超载</b><p>总载重超过 8 时，偶数层 +1</p></section><section className="pressure-relief"><small>会降低压力</small><b>音乐家开始演奏</b><p>车内至少 4 人时，每层 −1</p><b>护士进行安抚</b><p>每逢偶数层 −1</p><em>加压与安抚在同一层可相互抵消，压力最低为 0。</em></section></div><div className={`pressure-now forecast-${pressurePreview.tone}`}><small>按你现在的站位</small><b>{pressurePreview.summary}</b></div></DialogContent></Dialog>
