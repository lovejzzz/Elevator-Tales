...
export default function ElevatorGame() {
  ...
  return <main className={`game-shell ${cooperationRelief(run) ? 'has-contract' : ''} ${difficultyTier(run.floor) % 2 ? 'phase-dawn' : ''}`}>
    ...
    <section className="game-grid">
      <aside className="status-rail">
        ...
        <div data-metric="stress" className={`meter-card pressure ${agitated || pressurePreview.tone === 'danger' ? 'meter-danger' : ''}`} title={pressurePreview.summary}>
          <div><Gauge aria-hidden="true" /><span className="meter-label"><span className="sr-only">躁动</span><button className="meter-help" onClick={() => setPressureHelp(true)} aria-label="查看躁动规则"><HelpCircle /></button></span><b><AnimatedNumber value={run.stress} /></b></div>
        ...
        <div data-metric="coins" className="score-card wallet-card"><Coins aria-hidden="true" /><span className="sr-only">可用金币</span><strong><AnimatedNumber value={run.coins} /></strong><MetricResponse metric="coins" event={metricEvent} /><span className="mobile-shop-note">{nextShop - run.floor} 站到商店</span><small>本班累计赚取 {run.earned}<br />{nextShop} 层商店 · 还剩 {nextShop - run.floor} 站</small></div>
      ...
      <section className={`elevator-stage doors-${doors} ${activeRider ? 'is-placing' : ''} ${agitated ? 'cabin-agitated' : ''}`} aria-label="电梯座舱" aria-busy={doors !== 'open'}>
        ...
        {outlook && <div className={`adjacency-key shift-outlook ${(run.floor+1)%10!==0 ? 'peak-outlook' : ''}`}><span>{outlook}</span><span className="connection-legend">绿线协作 · 红虚线冲突</span></div>}
    ...
    <footer className="footer-line"><span>ELV–07 / v7.0</span><i /><span>THE CITY NEVER REALLY SLEEPS</span></footer>
