...
export default function ElevatorGame() {
  ...
  const edgeLabels=edgeView.flatMap(e=>{
    ...
    const severerParts=severerAboard?[e.greenSyms.length>0&&<span key="sg" className="ll-part ll-agitation ll-severer" title={zhUI?'剪线婆：每条绿线每层 +1 躁动':'Severer: +1 agitation per green link a floor'}><Scissors aria-hidden="true"/><Flame aria-hidden="true"/>+{e.greenSyms.length*DARK_LEGEND_RULES.severerPerGreen}</span>,edgeClashes>0&&<span key="sr" className="ll-part ll-coin ll-severer" title={zhUI?'剪线婆：每条红线每层 +3 币':'Severer: +3 coins per red link a floor'}><Scissors aria-hidden="true"/>+{edgeClashes*DARK_LEGEND_RULES.severerPerRed} {coinWord}</span>]:[];
    const parts=[...severerParts,e.greenSyms.length>0&&<span key="g" className="ll-part ll-sym" title={e.greenSyms.map(s=>symbolTitle(s,zhUI)).join('\n')}>{e.greenSyms.map(s=><span key={s} className={`sym-ink sym-${s}`}><SymbolIcon symbol={s} size={13} /></span>)}</span>, e.partnership&&<span key="p" className="ll-part ll-agitation"><Flame aria-hidden="true"/>+1</span>, e.shownConflict&&<span key="c">{conflictLabel(e.shownConflict.effect)}</span>, e.steal==='box'?<span key="s" className="ll-part ll-steal"><Package aria-hidden="true"/>{zhUI?'偷纸箱':'takes box'}</span>:e.steal?<span key="s" className="ll-part ll-steal"><Coins aria-hidden="true"/>+{e.steal} {coinWord}</span>:null].filter(Boolean);
    ...
    const tip:string[]=[
      ...
      ...e.greenSyms.map(k=>zhUI?`${nameA}和${nameB}都有「${sn(k)}」→ 绿线：${symbolEffectText(k,true)}/层（车里「${sn(k)}」每层只算一次，连成形状加级）`:`${nameA} and ${nameB} both carry ${sn(k)} → green link: ${symbolEffectText(k,false)} a floor (${sn(k)} counts once a floor; shapes add levels)`),
