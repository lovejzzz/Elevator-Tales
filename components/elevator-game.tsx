...
import { EMPTY_UPGRADES, failureLesson, hasNeighbour, initialRun, installUpgrade, isFreeReseat, makeOffers, neighbourCount, neighbours, resolveFloor, totalWeight, unlockedAt, upgradeChoices, type Rider, type RunState, type UpgradeCrisis } from '@/lib/game-engine';
...
export default function ElevatorGame() {
  ...
  return <main className={`game-shell ${run.floor >= 50 ? 'phase-dawn' : ''} ${run.status === 'won' ? 'shift-won' : ''}`}>
    ...
    <footer className="footer-line"><span>ELV–07 / v4.0</span><i /><span>THE CITY NEVER REALLY SLEEPS</span></footer>
    ...
    <Dialog open={run.status === 'upgrade'}><DialogContent className={`story-dialog upgrade-dialog ${upgradeCrisis ? 'upgrade-crisis' : ''}`} showCloseButton={false}><p className="dialog-kicker">FLOOR {run.floor} · MAINTENANCE STOP</p><DialogHeader><DialogTitle>{upgradeCrisis ? '维修层危机：选择救援' : '选择一项轿厢升级'}</DialogTitle><DialogDescription>{upgradeCrisis ? `${upgradeCrisis === 'energy' ? '能源已经耗尽' : '压力已经达到上限'}。选择标有“可挽救本班”的升级才能继续。` : '先看路线定位，再看本局的实际变化。生存、控场与高分没有永远正确的答案。'}</DialogDescription></DialogHeader><div className="upgrade-grid">{choices.map((key) => { const rescue = rescuesCrisis(key, upgradeCrisis); return <button className={rescue ? 'crisis-rescue' : ''} key={key} onClick={() => chooseUpgrade(key)}>{rescue && <span className="upgrade-rescue">EMERGENCY · 可挽救本班</span>}<Sparkles /><span className="upgrade-card-head"><small>{UPGRADES[key].label}</small><i className={`upgrade-strategy strategy-${UPGRADES[key].tone}`}>{UPGRADES[key].strategy}</i></span><b>{UPGRADES[key].name}</b><p>{UPGRADES[key].description}</p><em>{upgradeImpact(key, run)}</em></button>; })}</div></DialogContent></Dialog>
