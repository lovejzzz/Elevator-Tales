...
import { EMPTY_UPGRADES, hasNeighbour, initialRun, installUpgrade, makeOffers, neighbourCount, neighbours, resolveFloor, totalWeight, unlockedAt, upgradeChoices, type Rider, type RunState } from '@/lib/game-engine';
...
function stressForecast(state: RunState, weight: number): StressForecast {
  ...
  state.cabin.forEach((rider, slot) => {
    ...
    switch (rider.kind) {
      ...
      case 'celebrity': if (nextFloor % 2 === 0 && neighbourCount(state.cabin, slot) > 1) { low += 1; high += 1; celebrities += 1; } break;
...
export default function ElevatorGame() {
  ...
  const depart = useCallback(() => {
    ...
    setTimeout(() => { setRun((current) => { const resolved = resolveFloor(current); if (resolved.status === 'playing') setOffers(makeOffers(resolved.floor, resolved.upgrades)); playTone(sound, resolved.status === 'lost' ? 'danger' : resolved.status === 'won' ? 'victory' : 'arrive'); return resolved; }); setDoors('open'); busyRef.current = false; }, 920);
  }, [locked, sound]);
  ...
  const chooseUpgrade = (key: UpgradeKey) => {
    setRun((current) => {
      ...
      if (updated.status === 'playing') setOffers(makeOffers(current.floor, updated.upgrades)); return updated;
  ...
  return <main className={`game-shell ${run.floor >= 50 ? 'phase-dawn' : ''} ${run.status === 'won' ? 'shift-won' : ''}`}>
    ...
    <section className="game-grid">
      <aside className="status-rail">
        ...
        <div className="meter-card energy" title={energyPreview.summary}><div><BatteryCharging /><span>能源</span><b>{run.energy}</b></div><div className="meter-track"><i style={{ width: `${Math.max(0, Math.min(100, run.energy / run.energyCap * 100))}%` }} /></div><small>NEXT {energyPreview.range} · {run.energyCap} MAX</small>{run.lastEnergy.sources.length > 0 && <div className={`energy-receipt ${run.lastEnergy.delta > 0 ? 'gained' : run.lastEnergy.delta < 0 ? 'spent' : 'balanced'}`} key={run.floor} aria-live="polite"><b>{run.lastEnergy.delta === 0 ? '本层持平' : `本层 ${signedDelta(run.lastEnergy.delta)}`}</b><span>{energySummary}</span></div>}</div>
        <div className="meter-card pressure" title={pressurePreview.summary}><div><Gauge /><span>压力</span><b>{run.stress}</b></div><div className="meter-track"><i style={{ width: `${Math.min(100, run.stress / run.stressCap * 100)}%` }} /></div><small>NEXT {pressurePreview.range} · {run.stressCap} LIMIT</small>{run.lastPressure.sources.length > 0 && <div className={`pressure-receipt ${run.lastPressure.delta > 0 ? 'rose' : run.lastPressure.delta < 0 ? 'fell' : 'balanced'}`} key={run.floor} aria-live="polite"><b>{run.lastPressure.delta === 0 ? '本层抵消' : `本层 ${signedDelta(run.lastPressure.delta)}`}</b><span>{pressureSummary}</span></div>}</div>
        <div className={`load-card ${weight > 8 ? 'load-warn' : ''}`}><Weight /><span>载重</span><b>{weight} / {run.weightCap}</b></div>
        <div className={`score-card ${rankedUp ? 'rank-up' : ''}`}><Coins /><span>本次收入</span><strong>{run.coins}</strong><progress className="rank-progress" aria-label={`${rank.grade}级进度`} max={100} value={rankProgress} /><small><b>{rank.grade}</b><span className="rank-full"> {rank.name} · {rank.next ? `距 ${rank.next.grade} 级 ${rank.next.min - run.coins}` : '最高评级'} · 最佳 {bestCoins}</span><span className="rank-compact"> · {rank.next ? `距 ${rank.next.grade} ${rank.next.min - run.coins}` : '最高级'} · 最佳{bestCoins}</span></small>{run.lastEarnings.total > 0 && <div className="earning-receipt" key={run.floor} aria-live="polite"><b>{rankedUp ? `晋升 ${rank.grade} 级 · ` : ''}本层 +{run.lastEarnings.total}</b><span>{earningSummary}</span></div>}</div>
      ...
      <section className={`elevator-stage doors-${doors}`} aria-label="电梯座舱">
        ...
        <div className="cabin-message" aria-live="polite"><Sparkles /><span>{run.message}</span></div><div className="swap-status">{pendingOfferId ? '点一个空位安排乘客' : run.swapped ? <><LockKeyhole /> 本层换位已用</> : selectedSlot !== null ? '再选一个站位完成交换' : '拖拽人物安排站位 · 有效组合会亮起'}</div>
      </section>
    ...
    <footer className="footer-line"><span>ELV–07 / v3.0</span><i /><span>THE CITY NEVER REALLY SLEEPS</span></footer>
