...
import { SYMBOLS, SYMBOL_KEYS, greenBySymbol, symbolEdges, symbolEffectText, symbolShapes, symbolTitle, symbolsOf, type SymbolKey } from '@/lib/symbols';
...
function SymbolBadges({ rider, cabin, slot, locale }: { rider: Rider; cabin: Array<Rider | null>; slot: number; locale: GameLocale }) {
  const zh = locale === 'zh';
  ...
  return <span className="sym-badges">{syms.map((s: SymbolKey) => <span key={s} className={`sym sym-${s}`} title={symbolTitle(s, zh)} aria-label={zh ? SYMBOLS[s].zh : SYMBOLS[s].en}><SymbolIcon symbol={s} /></span>)}</span>;
}
