...
export function PassengerCardFace({ rider, run, action, locale }: { rider: Rider; run: RunState; action?: string; locale: GameLocale }) {
  ...
  return <span className="unified-passenger-summary compact-card" data-no-translate>
    <span className="cc-head"><Portrait kind={rider.kind}/><span className="cc-title"><strong>{riderName(rider.kind,locale)}</strong><span className="cc-sub">
      ...
      {rider.volatile&&<span className="cc-tag cc-tag-risk" title={zh?`高危乘客：车费 +${HIGH_RISK_BONUS}，在车上每层 +1 躁动`:`High-risk rider: fare +${HIGH_RISK_BONUS}, +1 agitation per floor aboard`}><Flame aria-hidden="true" />{zh?`高危：车费+${HIGH_RISK_BONUS} 躁动+1/层`:`High risk: fare +${HIGH_RISK_BONUS}, +1/floor`}</span>}
    ...
    {legend ? <span className="cc-values cc-legend-values"><span>{zh?'不付车费 · 不耗电':'No fare · no power'}{rider.kind==='medium'?'':''}</span></span> : <span className="cc-values">
      ...
      {net!==null&&<span className={`cc-net ${net>0?'is-pos':net<0?'is-neg':''}`} title={zh?'单独带这位乘客大约赚多少：车费 − 全程耗电×充电价 − 全程躁动×3。不含邻座加成，配对/组合的人物通常显示亏。':'Net ≈ fare − trip power × charge price − trip agitation × 3, before neighbour bonuses. Positive means worth carrying even alone.'}>{zh?(net>0?`单独 赚 ${net}`:net<0?`单独 亏 ${-net}`:'单独 持平'):(net>0?`Alone +${net}`:net<0?`Alone −${-net}`:'Alone ±0')}</span>}
    </span>}
