...
export default function ElevatorGame() {
  ...
  const content = <main className={`game-shell ${cooperationRelief(run) ? 'has-contract' : ''} ${difficultyTier(run.floor) % 2 ? 'phase-dawn' : ''}`}>
    ...
    <section className="game-grid">
      <aside className="status-rail">
        ...
        <div data-metric="stress" className={`meter-card pressure ${agitated || pressurePreview.tone === 'danger' ? 'meter-danger' : ''}`} title={pressurePreview.summary}>
          ...
          <span className="mobile-meter-cap">上限 {run.stressCap}</span><span className="mobile-agitation-state">再 +{Math.max(0,run.stressCap-run.stress)} 失控</span><small className="rail-forecast"><span>下一站 <b>{pressurePreview.range}</b></span><span>上限 {run.stressCap}</span></small><div className="agitation-state"><b>只计算人物</b><span>每位到站：本层最多 −1</span></div>
        </div>
    ...
    <Dialog open={help} onOpenChange={setHelp}><DialogContent className="story-dialog manual-dialog"><p className="dialog-kicker">ENDLESS SHIFT MANUAL</p><DialogHeader><DialogTitle>值班手册</DialogTitle><DialogDescription>楼层就是成绩；金币是购买升级的预算。没有最后一层。</DialogDescription></DialogHeader><div className="manual-grid">
      ...
      <div><b>十层补给</b><p>初始{INITIAL_ENERGY}电、容量{ENERGY_CAPACITY}。每十层可用金币充电或买升级；两者争用同一笔预算。高危乘客多赚 {HIGH_RISK_BONUS} 金币，但每层多 +1 躁动。</p></div><div><b>协作、冲突与堆叠</b><p>绿线表示协作：本人到站时，每条仍连接的绿线额外 +{cooperationBonus(run)} 金币，多条逐条叠加；有任意绿线时免除该人物全部邻座冲突。红线每层 +1 躁动。</p></div><div><b>到层请离</b><p>选中车内人物，打开人物详情后请离。赔偿4+剩余站数×2金币，不结算到站奖励。本层刚上车仍可免费撤回。</p></div>
    ...
    <Dialog open={run.status === 'upgrade'}><DialogContent className={`story-dialog upgrade-dialog ${upgradeCrisis ? 'upgrade-crisis' : ''}`} showCloseButton={false}>
      ...
      {leaveArmed && <p className="shop-warning">当前电量连到下个补给站的基础运转都不够，人物还会额外耗电。再点一次确认带风险离开。</p>}<Button className="story-primary" onClick={finishShopping}>{upgradeCrisis ? '无力修复 · 结束本班' : leaveArmed ? '确认少电离开 · 自担风险' : '离开商店 · 继续上行'}<ArrowUp /></Button>
    </DialogContent></Dialog>
