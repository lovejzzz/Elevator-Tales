...
export default function ElevatorGame() {
  ...
  const loverResponse = offers.some((rider) => rider.calledByLover);
  ...
  return <main className={`game-shell ${run.floor >= 50 ? 'phase-dawn' : ''} ${run.status === 'won' ? 'shift-won' : ''}`}>
    ...
    <footer className="footer-line"><span>ELV–07 / v3.7</span><i /><span>THE CITY NEVER REALLY SLEEPS</span></footer>
    ...
    <Dialog open={run.status === 'upgrade'}><DialogContent className="story-dialog upgrade-dialog" showCloseButton={false}><p className="dialog-kicker">FLOOR {run.floor} · MAINTENANCE STOP</p><DialogHeader><DialogTitle>选择一项轿厢升级</DialogTitle><DialogDescription>先看路线定位，再看本局的实际变化。生存、控场与高分没有永远正确的答案。</DialogDescription></DialogHeader><div className="upgrade-grid">{choices.map((key) => <button key={key} onClick={() => chooseUpgrade(key)}><Sparkles /><span className="upgrade-card-head"><small>{UPGRADES[key].label}</small><i className={`upgrade-strategy strategy-${UPGRADES[key].tone}`}>{UPGRADES[key].strategy}</i></span><b>{UPGRADES[key].name}</b><p>{UPGRADES[key].description}</p><em>{upgradeImpact(key, run)}</em></button>)}</div></DialogContent></Dialog>
