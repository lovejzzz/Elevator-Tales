...
import { AlarmClock, ChevronsUp, ArrowUp, Layers, Package, PackageCheck, Pill, ShieldCheck, UserMinus, BatteryCharging, BookOpen, Check, Coins, Flame, HelpCircle, History, Info, LockKeyhole, Music2, RotateCcw, Route, Link2, Timer, Scissors, Sparkles, Volume2, VolumeX, X } from 'lucide-react';
...
import { energyForecast, floorPreview, sectorForecast, shopAgitationRoom, stressForecast, type PreviewLine } from '@/lib/game-forecast';
...
import { emergencyUnitPrice, BOX_LINES, BOX_LINE_LABELS, BOX_MAX_LEVEL, boxTotal, chargeCost as boxChargeCost, chargeUnitPrice, EARLY_CHARGE, LATE_CHARGE, affordableUnits, type BoxLine } from '@/lib/power-box';
...
import { boardNet, pairedNet } from '@/lib/net-value';
...
/** v10.2.10 (playtest: a new link showed on the cabin but nothing moved on the rail): each rail box lists, line by line,
 * what changes its number on the next ascent. `good` says which direction is good news for the colour. */
...
const KEEPSAKE_EN: Record<KeepsakeKey, [string, string]> = {
  ...
  vinyl: ['Vinyl Record', 'Every Musician becomes a Master Musician: fare doubled, +6 coins a floor each at medium agitation (a Musician +2), pulls agitation up to 3 toward medium (2); they appear from floor 1, twice as often.'],
...
// v10.1.2: a keepsake is its picture and name; the effect shows on hover or tap.
function KeepsakeChip({ keepsake, locale, inCard = false }: { keepsake: KeepsakeKey; locale: GameLocale; inCard?: boolean }) {
  // Inside a card (itself a <button>) the chip is a plain span that shows its tooltip on hover.
  if (inCard) return <span className={`keepsake-chip keepsake-${keepsake}`}><KeepsakeIcon keepsake={keepsake} /><b>{keepsakeLabel(keepsake, locale)}</b><span className="keepsake-tip" role="tooltip">{keepsakeText(keepsake, locale)}</span></span>;
  return <button type="button" className={`keepsake-chip keepsake-${keepsake}`} aria-label={`${keepsakeLabel(keepsake, locale)}：${keepsakeText(keepsake, locale)}`}><KeepsakeIcon keepsake={keepsake} /><b>{keepsakeLabel(keepsake, locale)}</b><span className="keepsake-tip" role="tooltip">{keepsakeText(keepsake, locale)}</span></button>;
}
...
const ABYSS_EVENT_TEXT: Record<AbyssEventKind, { zh: [string, string]; en: [string, string] }> = {
  ...
  market: { zh: ['夜市', '门外有个摊位，卖两件夜市独有的贵货和一件商店道具'], en: ['Night market', 'A stall outside the doors sells two pricey goods found nowhere else and one shop item'] },
};
// v9.21.1 night-market goods reuse their shop item's painting, tinted (.item-icon.is-night).
const itemArt = (key: ItemKey) => shopIcon(`item-${ITEMS[key].art ?? key}`);
...
// v10: a rider's two symbols as icons (name and rule in the tooltip). A Mystery shows “?” until revealed; a Mimic shows
// the symbols of the rider above him, or “↑” when nobody is there.
// v10.1.2: any symbol icon opens the codex for its symbol (what it does, and which riders carry it).
const SymbolCodexContext = createContext<(symbol: SymbolKey) => void>(() => {});
function SymbolBadges({ rider, cabin, slot, locale, withEffects = false }: { rider: Rider; cabin: Array<Rider | null>; slot: number; locale: GameLocale; withEffects?: boolean }) {
  const zh = locale === 'zh', openCodex = useContext(SymbolCodexContext);
  if (rider.kind === 'parcel' || isAnyLegend(rider.kind)) return null;
  if (rider.kind === 'mystery' && !rider.revealed) return <span className="sym-badges"><span className="sym sym-hidden" title={zh ? '身份揭晓后才知道他的符号' : 'His symbols show once he is revealed'}>?</span></span>;
  const syms = symbolsOf(rider, cabin, slot);
  if (rider.kind === 'mimic' && !syms.length) return <span className="sym-badges"><span className="sym sym-copy" title={zh ? '复制正上方乘客的符号' : 'Copies the symbols of the rider directly above'}>↑</span></span>;
  // v10.1.2: on cards each symbol shows what its green link does (“安静 省1电”); seats keep the bare icons.
  return <span className={withEffects ? 'sym-row' : 'sym-badges'}>{syms.map((s: SymbolKey) => <span key={s} className={withEffects ? `sym-chip sym-${s}` : 'sym-slot'}>{/* oxlint-disable-next-line jsx-a11y/prefer-tag-over-role -- it sits inside a card or seat <button>, so it cannot be a <button> itself */}
    <span role="button" tabIndex={0} className={`sym sym-${s} sym-open`} title={`${symbolTitle(s, zh)}${zh ? '（点开看这一类人）' : ' (click for everyone who carries it)'}`} aria-label={zh ? SYMBOLS[s].zh : SYMBOLS[s].en}
    onPointerDown={e => e.stopPropagation()} onMouseDown={e => e.stopPropagation()} onClick={e => { e.stopPropagation(); e.preventDefault(); openCodex(s); }} onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') { e.stopPropagation(); e.preventDefault(); openCodex(s); } }}><SymbolIcon symbol={s} /></span>{withEffects && <span className="sym-chip-text"><b>{zh ? SYMBOLS[s].zh : SYMBOLS[s].en}</b>{zh ? symbolEffectText(s, true).replace(/\s+/g, '') : symbolEffectText(s, false)}</span>}</span>)}</span>;
}
...
function riderState(cabin: Array<Rider | null>, slot: number, bonus: number, agitation: number, floor = 0): { label: string; tone: 'active' | 'warn' | 'neutral' } | null {
  ...
  switch (rider.kind) {
    ...
    default: return bond.supported ? {label:`绿线 ${bond.supportCount} 条`,tone:'active'} : null;
  }
}
...
function upgradeImpact(key: UpgradeKey, run: RunState): string {
  ...
  switch (key) {
    case 'battery': return `热闹和江湖每级绿线每层 +${cooperationBonus(run) - 1} → +${cooperationBonus(preview) - 1} 金币。`;
...
const MANUAL: Array<[string, string, string, string]> = [
  ...
  ['符号与链接', '每位乘客有两个符号。邻座有相同符号＝绿线；有相反符号（热闹↔安静、秩序↔江湖、人间↔幽冥）＝红线；同一对邻座的相同和相反符号一对一抵消。车里每种符号只要有一条绿线，每层就按上面的效果生效一次；同一符号连成一排再加 1 级、四人方块加 2 级、满车加 4 级。每条红线每层 +1 躁动。午夜后的暗黑版多是热闹、江湖、幽冥，和白天的乘客容易起冲突。', 'Symbols and links', 'Every rider carries two symbols. Neighbours who share a symbol = green link; neighbours with opposite symbols (Lively↔Quiet, Order↔Street, Hearth↔Spirit) = red link; a pair’s matches and clashes cancel one for one. Each symbol with at least one green link in the cabin works once a floor, as listed above; a row of one symbol adds a level, a 2×2 square two, a full cabin four. Each red link adds 1 agitation a floor. After midnight the dark riders are mostly Lively, Street and Spirit, and clash easily with daytime riders.'],
  ['卡片上的数字', '画像下面一排：车费、路程（还坐几站）、每层耗电、躁动。每层耗电＝电梯运转＋乘客耗电−节能（安静和幽冥的绿线也省电）。31 层起上车的乘客基础车费打 75 折，卡上写的就是折后价。', 'The numbers on a card', 'The row under the portrait: fare, trip (stops left), power per floor and agitation. Power per floor = motor + riders − savings (Quiet and Spirit green links save power too). Riders who board from 31F on pay 75% of their base fare, and the card shows that price.'],
  ['电量', '电量耗尽即结束。商店充电 2 金币/电，40 层前打 6 折，61 层起贵 25%；两次商店之间可以途中补电，4 金币/电，每十层最多 20 电。坐满 6 人时风扇每层多耗 1 电。', 'Power', 'Run out and the shift ends. Shops charge 2 coins per power: 40% off until 40F, 25% more from 61F. Between shops you can charge in transit at 4 coins, up to 20 per ten floors. With all 6 seats taken the fans cost 1 more power per floor.'],
...
const PRESSURE_RELIEF: Array<[string, string, string, string]> = [
  ...
  ['秩序与 人间', '车里有这两种符号的绿线时，每种每层 −1 躁动（连成一排、方块或满车更多）。', 'Order and Hearth', 'A green link of either symbol calms 1 a floor each (more for a row, a square or a full cabin).'],
...
/** v10.2.3 (playtest: dragging stuttered): the closed changelog, archive and manual dialogs were rebuilt on every game render
 * (thousands of elements, then walked again by the English translation), about 0.6s a render in development. Their bodies
 * are now built only while open, and kept for the closing animation. */
function WhenOpen({ open, language, render }: { open: boolean; language: GameLocale; render: () => ReactNode }) {
  const [shown, setShown] = useState(open);
  if (open && !shown) setShown(true);
  useEffect(() => { if (open) return; const timer = setTimeout(() => setShown(false), 400); return () => clearTimeout(timer); }, [open]);
  // The body comes in as a `render` prop, not children: localizeTree walks children and would mangle a function child.
  return shown ? <>{localizeTree(render(), language)}</> : null;
}

const LEGEND_PASS_HINT = '传奇不用请离券：点开他的人物详情，「请离」就能免费让他下车。', LEGEND_PASS_KINGPIN = '黑老大不能用请离券：在人物详情里请离他，要付一笔赔偿。';
...
export default function ElevatorGame() {
  ...
  const [codex, setCodex] = useState<SymbolKey | null>(null);
  ...
  // v10.1.2 run record v2: every player action between two ascents (charge, calm, item, dismissal, shop purchase…).
  const actionsRef = useRef<Array<Record<string, unknown>>>([]);
  ...
  const reportMetrics = useCallback((before: RunState, after: RunState, label: string) => {
    if (!label.endsWith('到站结算') && (after.coins !== before.coins || after.energy !== before.energy || after.stress !== before.stress || (after.items?.length ?? 0) !== (before.items?.length ?? 0)))
      actionsRef.current.push({ action: label, coins: after.coins - before.coins, energy: after.energy - before.energy, stress: after.stress - before.stress });
  ...
  const stressAdvice = (() => {
    ...
    if (run.calmCharge) tips.push(zh ? '先用手动调节（免费 −3）' : 'use the free manual relief (−3)');
  ...
  // v10.2.9: computed on every fatal floor, not only when calming falls short: the free manual relief or one dismissal is
  // often far cheaper than calming alone (76F playtest: 72 coins of calming at 66F with the manual relief unused).
  const calmRescue = useMemo(() => (stressFatal ? calmRescuePlan(run) : null), [run, stressFatal]);
  const calmPlanShown = calmRescue && (calmLeft < calmNeed || calmRescue.manual || calmRescue.remove.length > 0) ? calmRescue : null;
  ...
  // v10.2.3 (playtest: shopping first, then leaving at agitation 13/8 with 33 coins, ended the run): in a crisis no purchase may
  // touch the coins the minimum rescue needs (the engine refuses it too, keepsShopRescue); the button says why.
  const rescueCost=upgradeCrisis?emergencyRepairPlan(run).cost:0, rescueSpare=upgradeCrisis?run.coins-rescueCost:Infinity;
  const spendLocked=(price:number,rescues=false)=>!rescues&&price>0&&price>rescueSpare;
  const rescueLockNote=language==='zh'?`先留 ${rescueCost} 币抢救`:`Keep ${rescueCost} coins for the rescue`;
  ...
  const clickSlot = (slot: number) => {
    if (locked) return;
    if (itemAim) { const target = run.cabin[slot]; if (target && itemUsable(run, itemAim, target)) applyBagItem(itemAim, target.id); else {
      // v10.2.7 (human playtest: “the Exit Pass would not take the Severer — bug?”): legends are never a pass target because the
      // ordinary dismissal already lets them off for free; say so instead of silently cancelling.
      if (target && itemAim === 'dismiss' && isAnyLegend(target.kind)) setRun(current => ({ ...current, message: target.kind === 'kingpin' ? LEGEND_PASS_KINGPIN : LEGEND_PASS_HINT }));
      setItemAim(null); } return; }
    if (pendingOfferId) {
  ...
  const depart = useCallback(() => {
    ...
    journeyTimers.current = [
      ...
      setTimeout(() => {
        ...
        { const led = symbolCoins(run), lines = (l: ChangeLine[]) => l.filter(x => x.amount).map(x => [x.label, x.amount]);
          recordRef.current.push({ floor: run.floor, energy: run.energy, stress: run.stress, coins: run.coins, cabin: run.cabin.map(r => r ? [r.kind, r.destination - run.floor] : null),
            seats: run.cabin.map((r, i) => r ? { kind: r.kind, stops: r.destination - run.floor, symbols: symbolsOf(r, run.cabin, i), ...(r.volatile ? { volatile: true } : {}), ...(r.big ? { big: r.big } : {}), ...(r.stash ? { stash: r.stash } : {}) } : null),
            links: { active: greenBySymbol(run.cabin), red: led.red, coins: led.coins, powerSaved: energyBreakdown(run).symbol, calm: -led.agitationLines.reduce((n, x) => n + x.amount, 0) },
            offers: offers.map(o => o.kind), actions: actionsRef.current.splice(0), arrivals: (resolved.lastArrivals ?? []).map(a => [a.kind, a.coins]),
            income: lines(resolved.lastEarnings.sources), power: lines(resolved.lastEnergy.sources), agitation: lines(resolved.lastPressure.sources),
            events: { outbursts: (resolved.lastOutbursts ?? []).map(i => run.cabin[i]?.kind), incident: resolved.lastIncident?.kind ?? null, thefts: resolved.lastThefts?.length ?? 0, corrupted: (resolved.lastCorruption ?? []).map(c => [c.from, c.to]) },
            after: { energy: resolved.energy, stress: resolved.stress, coins: resolved.coins, status: resolved.status } }); }
  ...
  // v10.2.7 (human playtest, 61–69F): with the Severer aboard every green link also adds agitation and every red link pays
  // coins, but the labels only showed the usual effects, so the player kept building green links. The labels now say so.
  const severerAboard=linkCabin.some(r=>r?.kind==='severer');
  const edgeLabels=edgeView.flatMap(e=>{
    const edgeClashes=severerAboard?(symbolEdges(linkCabin).find(x=>x.first===e.first&&x.second===e.second)?.clashes.length??0):0;
    const severerParts=severerAboard?[e.greenSyms.length>0&&<span key="sg" className="ll-part ll-agitation ll-severer"><Scissors aria-hidden="true"/><Flame aria-hidden="true"/>+{e.greenSyms.length*DARK_LEGEND_RULES.severerPerGreen}</span>,edgeClashes>0&&<span key="sr" className="ll-part ll-coin ll-severer"><Scissors aria-hidden="true"/>+{edgeClashes*DARK_LEGEND_RULES.severerPerRed} {coinWord}</span>]:[];
    const parts=[...severerParts,e.greenSyms.length>0&&<span key="g" className="ll-part ll-sym">{e.greenSyms.map(s=><span key={s} className={`sym-ink sym-${s}`}><SymbolIcon symbol={s} size={13} /></span>)}</span>, e.partnership&&<span key="p" className="ll-part ll-agitation"><Flame aria-hidden="true"/>+1</span>, e.shownConflict&&<span key="c">{conflictLabel(e.shownConflict.effect)}</span>, e.steal==='box'?<span key="s" className="ll-part ll-steal"><Package aria-hidden="true"/>{zhUI?'偷纸箱':'takes box'}</span>:e.steal?<span key="s" className="ll-part ll-steal"><Coins aria-hidden="true"/>+{e.steal} {coinWord}</span>:null].filter(Boolean);
    if(!parts.length)return [];
    const [x1,y1]=CONNECTION_POINTS[e.first],[x2,y2]=CONNECTION_POINTS[e.second];
    // v10.1.2: hovering (or tapping) a link explains it — which symbols match or clash and what each does.
    const A=linkCabin[e.first],B=linkCabin[e.second],nameA=A?displayName(A,language):'',nameB=B?displayName(B,language):'';
    const sn=(k:SymbolKey)=>zhUI?SYMBOLS[k].zh:SYMBOLS[k].en;
    const clashes=symbolEdges(linkCabin).find(x=>x.first===e.first&&x.second===e.second)?.clashes??[];
    const tip:string[]=[
      ...(severerAboard&&e.greenSyms.length?[zhUI?`剪线婆在车上：这条绿线每层另外 +${e.greenSyms.length*DARK_LEGEND_RULES.severerPerGreen} 躁动（绿线原本的效果照旧）`:`The Severer is aboard: this green link also adds ${e.greenSyms.length*DARK_LEGEND_RULES.severerPerGreen} agitation a floor (its usual effect still applies)`]:[]),
      ...(severerAboard&&edgeClashes?[zhUI?`剪线婆在车上：这条红线每层 +${edgeClashes*DARK_LEGEND_RULES.severerPerRed} 币`:`The Severer is aboard: this red link pays ${edgeClashes*DARK_LEGEND_RULES.severerPerRed} coins a floor`]:[]),
      // v10.2.10 (playtest: a second Quiet link changed nothing and the player could not tell why): a symbol counts once a
      // floor, so the tip says when another link of it already counts, and the symbol's current level.
      ...e.greenSyms.map(k=>{const others=symbolEdges(linkCabin).filter(x=>x.shared.includes(k)&&!(x.first===e.first&&x.second===e.second)).length,lv=greenBySymbol(linkCabin)[k]??1;
        const shapes=symbolShapes(linkCabin).filter(x=>x.symbol===k).map(x=>zhUI?({row:'一排',square:'方块',full:'满车'})[x.kind]:({row:'a row',square:'a square',full:'the full cabin'})[x.kind]);
        const per=(t:string)=>t.startsWith('省')?`每层${t}`:`每层 ${t}`;
        if(!others&&lv===1)return zhUI?`${nameA}和${nameB}都有「${sn(k)}」→ 绿线：${per(symbolEffectText(k,true))}。`:`${nameA} and ${nameB} both carry ${sn(k)} → green link: ${symbolEffectText(k,false)} a floor.`;
        const dup=others?(zhUI?`车里共有 ${others+1} 条「${sn(k)}」绿线，但同一种符号每层只算一次，不按条数叠加。`:`There are ${others+1} ${sn(k)} links aboard, but a symbol counts once a floor, not once per link.`):'';
        const level=zhUI?`「${sn(k)}」现在共 ${lv} 级${shapes.length?`（连成${shapes.join('、')}）`:''}：${per(symbolEffectText(k,true,lv))}。`:`${sn(k)} is now level ${lv}${shapes.length?` (${shapes.join(', ')})`:''}: ${symbolEffectText(k,false,lv)} a floor.`;
        return zhUI?`${nameA}和${nameB}都有「${sn(k)}」→ 绿线。${dup}${level}`:`${nameA} and ${nameB} both carry ${sn(k)} → green link. ${dup}${dup?' ':''}${level}`;}),
      ...clashes.map(([a,b])=>{const [ka,kb]=(A&&symbolsOf(A,linkCabin,e.first).includes(a))?[a,b]:[b,a];return zhUI?`${nameA}的「${sn(ka)}」和${nameB}的「${sn(kb)}」相反 → 红线：每层 +1 躁动`:`${nameA}’s ${sn(ka)} opposes ${nameB}’s ${sn(kb)} → red link: +1 agitation a floor`;}),
      ...(e.partnership?[zhUI?'两个没人管的坏人挨着：每层各暂存金币，送达兑现，但 +1 躁动':'Two unguarded troublemakers side by side: each banks coins a floor, paid on delivery, but +1 agitation']:[]),
      ...(e.steal==='box'?[zhUI?'小偷盯上了旁边的纸箱：到站时顺走':'The Thief has an eye on the box beside him: he takes it when he leaves']:e.steal?[zhUI?`小偷每层从邻座身上顺走 ${e.steal} 币给你`:`The Thief lifts ${e.steal} ${coinWord} a floor from his neighbour for you`]:[]),
    ];
  ...
  const content = <main className={`game-shell ${cooperationRelief(run) ? 'has-contract' : ''} ${difficultyTier(run.floor) % 2 ? 'phase-dawn' : ''}`}>
    ...
    <section className="game-grid">
      <aside className="status-rail">
        ...
        <div data-metric="energy" className={`meter-card energy ${energyFatal ? 'meter-danger' : ''}`} title={`${energyPreview.summary} · ${language==='zh'?`运转 ${travelEnergyCost(run.floor+1)} 电/层`:`motor ${travelEnergyCost(run.floor+1)}/floor`}${nextIsShop?'':` · ${motorAdvanceNotice(run.floor)}`}`}><div><BatteryCharging aria-hidden="true" /><span className="rail-metric-name">电量</span><b><AnimatedNumber value={run.energy} /><span className="metric-cap">/{run.energyCap}</span></b></div><MetricResponse metric="energy" event={metricEvent} locale={language} /><PowerGauge value={run.energy} cap={run.energyCap} next={run.energy + energyPreview.lowDelta} danger={Math.max(1, -energyPreview.lowDelta)} locale={language} /><small className="rail-forecast rail-power-line" data-no-translate><span>{language==='zh'?'下一站':'Next'} <b className={energyFatal ? 'forecast-fatal' : ''}>{language==='zh'?energyPreview.range:translateGameText(energyPreview.range,'en')}</b></span>{run.status==='playing'&&<span className={`sector-forecast ${sector.failFloor!==null?'is-danger':sector.projected<8?'is-warn':''}`}>{language==='zh'?(sector.failFloor!==null?`${sector.failFloor} 层断电`:`到店剩 ${sector.projected}`):(sector.failFloor!==null?`out at ${sector.failFloor}F`:`${sector.projected} at shop`)}</span>}</small>
          {preview&&<MetricSources lines={preview.energy} locale={language} good="up" note={randomNote(energyPreview.lowDelta,energyPreview.highDelta,preview.energyDelta)} />}
        ...
        <div data-metric="stress" className={`meter-card pressure ${agitated || pressurePreview.tone === 'danger' ? 'meter-danger' : ''}`} title={pressurePreview.summary}>
          ...
          {preview&&<MetricSources lines={preview.stress} locale={language} good="down" note={[preview.stressUnused?(language==='zh'?`躁动最低是 0，其中 ${preview.stressUnused} 点降不下去`:`Agitation stops at 0; ${preview.stressUnused} of the relief goes unused`):'',randomNote(pressurePreview.lowDelta,pressurePreview.highDelta,preview.stressDelta)??''].filter(Boolean).join(language==='zh'?'；':'; ')||undefined} />}
          {run.status==='playing'&&run.calmCharge&&run.stress>0&&<div className="emergency-charge calm-charge" data-no-translate title={language==='zh'?'免费；每个商店补满':'Free; refills at every shop'}><button className={stressFatal?'is-urgent':''} disabled={locked} onClick={()=>{const next=applyCalmCharge(run);if(next===run)return;setRun(next);reportMetrics(run,next,'手动调节');playSfx(sound,'calm');}}>{language==='zh'?`手动调节 −${Math.min(3,run.stress)} · 免费`:`Manual relief −${Math.min(3,run.stress)} · free`}</button></div>}
        ...
        {run.status==='playing'&&run.cabin.some(Boolean)&&(()=>{const zh=language==='zh',lv=greenBySymbol(run.cabin),bonus=cooperationBonus(run)-1,reds=conflictLinks(run.cabin).length,act=SYMBOL_KEYS.filter(k=>lv[k]);
          // v10: what the cabin's symbol links do on this ascent — the heart of the new link rules, so it sits first.
          return <div className="link-ledger" data-no-translate><span className="link-ledger-label">{zh?'这一层的链接':'Links this floor'}</span>{act.length||reds?<span className="link-ledger-rows">{act.map(k=><span key={k} className={`link-ledger-item sym-${k}`} title={symbolTitle(k,zh)}><b className={`sym sym-${k}`}><SymbolIcon symbol={k} size={12} /></b>{zh?SYMBOLS[k].zh:SYMBOLS[k].en}{(()=>{const sh=symbolShapes(run.cabin).filter(x=>x.symbol===k).map(x=>zh?({row:'一排',square:'方块',full:'满车'})[x.kind]:({row:'row',square:'square',full:'full cabin'})[x.kind]);return sh.length?` · ${sh.join(zh?'、':', ')}`:'';})()}{lv[k]!>1?(zh?` · ${lv[k]}级`:` · level ${lv[k]}`):''}<em>{symbolEffectText(k,zh,lv[k],bonus)}</em></span>)}{reds>0&&<span className="link-ledger-item is-red"><b>✕</b>{zh?`红线 ×${reds}`:`Red links ×${reds}`}<em>{zh?`+${reds} 躁动`:`+${reds} agitation`}</em></span>}{run.cabin.some(r=>r?.kind==='severer')&&(greenLinks(run.cabin)||reds)>0&&<span className="link-ledger-item is-severer"><b><Scissors aria-hidden="true"/></b>{zh?'剪线婆':'Severer'}<em>{[greenLinks(run.cabin)?(zh?`绿线 +${greenLinks(run.cabin)*DARK_LEGEND_RULES.severerPerGreen} 躁动`:`green +${greenLinks(run.cabin)*DARK_LEGEND_RULES.severerPerGreen} agitation`):'',reds?(zh?`红线 +${reds*DARK_LEGEND_RULES.severerPerRed} 币`:`red +${reds*DARK_LEGEND_RULES.severerPerRed} coins`):''].filter(Boolean).join(zh?'，':', ')}</em></span>}</span>:<span className="link-ledger-empty">{zh?'让相同符号的人挨着坐，连出绿线':'Seat riders who share a symbol side by side for a green link'}</span>}</div>;})()}
        ...
        {run.status==='playing'&&run.marketFloor===run.floor&&run.marketStock&&<div className="night-market" data-no-translate><span className="item-bag-label">{language==='zh'?`夜市 · 道具 ${run.items?.length??0}/${ITEM_SLOTS}`:`Night market · items ${run.items?.length??0}/${ITEM_SLOTS}`}</span><div className="night-market-stall">{run.marketStock.map((card,i)=><button key={`${card.key}-${i}`} className={ITEMS[card.key].market?'is-night':undefined} disabled={locked||card.sold||run.coins<card.price||(run.items?.length??0)>=ITEM_SLOTS||spendLocked(card.price)} title={spendLocked(card.price)?rescueLockNote:undefined} onClick={()=>{const next=buyMarketItem(run,i);if(next!==run){reportMetrics(run,next,language==='zh'?'夜市':'Night market');setRun(next);playTone(sound,'upgrade');}}}><span className={`item-icon ${ITEMS[card.key].market?'is-night':''}`} style={{backgroundImage:`url(${itemArt(card.key)})`}} aria-hidden="true" /><span className="night-market-text"><span className="item-name">{language==='zh'?ITEMS[card.key].name:ITEMS[card.key].en}{ITEMS[card.key].market&&<em>{language==='zh'?'夜市货':'Night goods'}</em>}</span><small>{language==='zh'?ITEMS[card.key].zh:ITEMS[card.key].enText}</small></span><b>{card.sold?(language==='zh'?'已买':'Sold'):`${card.price}${language==='zh'?' 币':'c'}`}</b></button>)}</div></div>}
        {(run.items?.length??0)>0&&<div className="item-bag" data-no-translate><span className="item-bag-label">{language==='zh'?`道具 ${run.items!.length}/${ITEM_SLOTS}`:`Items ${run.items!.length}/${ITEM_SLOTS}`}</span><div className="item-bag-slots">{run.items!.map((key,i)=><button key={`${key}-${i}`} data-item={key} className={`item-chip ${itemAim===key?'is-aiming':''}`} disabled={locked||run.status!=='playing'||!itemReady(key)} onClick={()=>pickItem(key)} title={language==='zh'?`${ITEMS[key].name}：${ITEMS[key].zh}`:`${ITEMS[key].en}: ${ITEMS[key].enText}`}><span className={`item-icon ${ITEMS[key].market?'is-night':''}`} style={{backgroundImage:`url(${itemArt(key)})`}} aria-hidden="true" /><span className="item-name">{language==='zh'?ITEMS[key].name:ITEMS[key].en}</span></button>)}</div>{itemAim&&<p className="item-aim-hint">{language==='zh'?`${ITEMS[itemAim].name}：点一位发光的乘客（${ITEMS[itemAim].zh}）`:`${ITEMS[itemAim].en}: tap a glowing rider (${ITEMS[itemAim].enText})`}<button type="button" onClick={()=>setItemAim(null)}>{language==='zh'?'取消':'Cancel'}</button></p>}</div>}
      ...
      <section className={`elevator-stage doors-${doors} ${run.floor > DARK_RULES.midnightFloor || (run.floor === DARK_RULES.midnightFloor && run.status === 'playing') ? 'is-midnight' : ''} ${activeRider ? 'is-placing' : ''} ${agitated ? 'cabin-agitated' : ''} ${run.status==='playing'&&risk.fatal ? 'power-fatal' : run.status==='playing'&&run.energy + energyPreview.lowDelta <= LOW_POWER_FLICKER ? 'power-low' : ''}`} data-district={districtFor(run.floor).id} style={{ '--shake': (0.6 + 2.6 * Math.min(1, run.stress / Math.max(1, run.stressCap))).toFixed(2) } as CSSProperties} aria-label="电梯座舱" aria-busy={doors !== 'open'}>
        ...
        {outlook && <div className={`adjacency-key shift-outlook ${nextIsShop ? 'shop-next-outlook' : 'peak-outlook'}`}><span>{outlook}</span><span className="connection-legend">绿线：相同符号 · 红线：相反符号 · 紫箭头复制</span></div>}
      ...
      <aside className="arrival-panel">
        ...
        <div className="departure-controls">
          ...
          {run.status==='playing'&&doors==='open'&&occupied>0&&stressFatal&&!risk.fatal&&<div className={`power-alert is-fatal ${departArmed?'is-armed':''}`} role="alert" data-no-translate>
            ...
            <div className="power-alert-actions">{calmLeft>=calmNeed&&!calmPlanShown&&<button disabled={locked} onClick={()=>calm(calmNeed)}>{language==='zh'?`安抚 −${calmNeed} · ${calmNeed*calmPrice(run.floor)}币`:`Calm −${calmNeed} · ${calmNeed*calmPrice(run.floor)}c`}</button>}{calmLeft<1&&overtimeOk&&<button disabled={locked} onClick={overtimeCalm}>{language==='zh'?`加急安抚 −1 · ${overtimePrice}币`:`Overtime calm −1 · ${overtimePrice}c`}</button>}</div>
            {calmPlanShown?<div className="power-alert-rescue"><small>{(()=>{const p=calmPlanShown,zh=language==='zh',steps=[...(p.manual?[zh?'手动调节 −3（免费）':'manual relief −3 (free)']:[]),...p.remove.map(r=>zh?(r.paid?`请离${riderName(r.kind,'zh')}（${r.paid} 币${r.fare?`，少收 ${r.fare} 币车费`:''}）`:`撤回刚上车的${riderName(r.kind,'zh')}`):(r.paid?`dismiss the ${riderName(r.kind,'en')} (${r.paid}c${r.fare?`, loses a ${r.fare}c fare`:''})`:`withdraw the new ${riderName(r.kind,'en')}`)),...(p.calm?[zh?`安抚 −${p.calm}`:`calm −${p.calm}`]:[])];const cheaper=calmLeft>=calmNeed;const alone=calmNeed*calmPrice(run.floor);return zh?`${cheaper?'更省的办法':'能撑过'}：${steps.join('、')}。${cheaper?`只安抚要 ${alone} 币。`:''}`:`${cheaper?'Cheaper way':'You can make it'}: ${steps.join(', ')}.${cheaper?` Calming alone costs ${alone}c.`:''}`;})()}</small>{calmPlanShown.remove.every(r=>r.paid>0)&&<button disabled={locked} onClick={()=>{let next=calmPlanShown.manual?applyCalmCharge(run):run;for(const r of calmPlanShown.remove)next=dismissRider(next,r.id);if(calmPlanShown.calm)next=buyCalm(next,calmPlanShown.calm);if(next!==run){reportMetrics(run,next,language==='zh'?'请离并安抚':'Dismiss and calm');setRun(next);playTone(sound,'upgrade');playSfx(sound,'calm');}}}>{language==='zh'?`照此安排 · ${calmPlanShown.cost?`${calmPlanShown.cost}币`:'免费'}`:`Do it · ${calmPlanShown.cost?`${calmPlanShown.cost}c`:'free'}`}</button>}</div>:calmLeft<calmNeed&&<small className="power-alert-doomed">{stressAdvice}</small>}
          ...
          <p className={`panel-hint forecast-${forecastTone}`} aria-live="polite">{pendingOfferId ? '已选中乘客 · 请点电梯里的目标空位' : firstPairLesson && !firstPairActive ? '新手示例 · 让两位恋人成为邻座：两人都有人间和安静，会连出绿线' : pairHint ? pairHint.p.partner === pairHint.o.kind ? `两位${PASSENGERS[pairHint.o.kind].name}单看不划算，挨着坐就赚：一起带上。` : `${PASSENGERS[pairHint.o.kind].name}单看不划算，和${pairHint.p.partner === 'parcel' ? '纸箱' : PASSENGERS[pairHint.p.partner].name}挨着坐就赚：一起带上。` : cautiousHint ? '空着的座位也在耗运转电：多带几位，让相同符号的人挨着坐，常常能赚回来。' : departureForecast}</p>
        </div>
      </aside>
    </section>
    ...
    <Dialog open={inventoryOpen} onOpenChange={setInventoryOpen}><DialogContent className="story-dialog inventory-dialog">
      ...
      {(run.keepsakes?.length??0)>0&&<p className="keepsake-row" data-no-translate>{language==='zh'?'信物：':'Keepsakes: '}{run.keepsakes!.map(k=><KeepsakeChip key={k} keepsake={k} locale={language}/>)}</p>}
    ...
    <Dialog open={codex!==null} onOpenChange={open=>{if(!open)setCodex(null);}}><DialogContent className="story-dialog codex-dialog" data-no-translate>{codex&&(()=>{const zh=language==='zh',k=codex,opp=SYMBOLS[k].opposite,nm=(x:SymbolKey)=>zh?SYMBOLS[x].zh:SYMBOLS[x].en;
      const carriers=(Object.keys(RIDER_SYMBOLS) as PassengerKind[]).filter(kind=>RIDER_SYMBOLS[kind]!.includes(k));
      const cell=(kind:PassengerKind)=>{const open=discovered.includes(kind),other=RIDER_SYMBOLS[kind]!.find(x=>x!==k)!;return <div className={`codex-rider ${open?'':'locked'}`} key={kind}>{open?<Portrait kind={kind}/>:<span className="codex-blank" aria-hidden="true"/>}<b>{open?riderName(kind,language):'？？？'}</b>{open&&<span className="codex-other"><span className={`sym sym-${other}`}><SymbolIcon symbol={other} size={11}/></span>{nm(other)}</span>}</div>;};
      return <><p className="dialog-kicker">{zh?'符号':'SYMBOL'}</p><DialogHeader><DialogTitle className="codex-title"><span className={`sym sym-${k} codex-big`}><SymbolIcon symbol={k} size={26}/></span>{nm(k)}</DialogTitle><DialogDescription>{zh?`车里有一条「${nm(k)}」绿线，每层 ${symbolEffectText(k,true)}；多条只算一次，连成一排、方块、满车加级。和「${nm(opp)}」相反，挨着就是红线（每层 +1 躁动）。`:`With a ${nm(k)} green link in the cabin: ${symbolEffectText(k,false)} a floor. It counts once however many links there are; a row, a square or a full cabin adds levels. Opposite of ${nm(opp)}: sitting beside it makes a red link (+1 agitation a floor).`}</DialogDescription></DialogHeader>
        <h3 className="archive-subhead">{zh?`白天的乘客 · ${carriers.filter(x=>!isDark(x)&&discovered.includes(x)).length}/${carriers.filter(x=>!isDark(x)).length}`:`Day riders · ${carriers.filter(x=>!isDark(x)&&discovered.includes(x)).length}/${carriers.filter(x=>!isDark(x)).length}`}</h3>
        <div className="codex-grid">{carriers.filter(x=>!isDark(x)).map(cell)}</div>
        {carriers.some(x=>isDark(x))?<><h3 className="archive-subhead">{zh?`午夜后的暗黑版 · ${carriers.filter(x=>isDark(x)&&discovered.includes(x)).length}/${carriers.filter(x=>isDark(x)).length}`:`After midnight · ${carriers.filter(x=>isDark(x)&&discovered.includes(x)).length}/${carriers.filter(x=>isDark(x)).length}`}</h3>
        <div className="codex-grid">{carriers.filter(x=>isDark(x)).map(cell)}</div></>:<p className="codex-note">{zh?`午夜后的暗黑版没有人带「${nm(k)}」：过了 60 层，它只能靠留在车上的白天乘客。`:`No dark rider carries ${nm(k)}: after 60F it lives only on the day riders still aboard.`}</p>}
        <p className="codex-note">{zh?'复制人显示正上方乘客的符号；百变人的符号随机；神秘人揭晓身份后才有符号。':'The Mimic shows the symbols of the rider above him; the Shifter’s are random; the Mystery has none until revealed.'}</p></>;})()}</DialogContent></Dialog>
    <Dialog open={archive} onOpenChange={setArchive}><DialogContent className="story-dialog archive-dialog"><WhenOpen open={archive} language={language} render={()=><><p className="dialog-kicker">PASSENGER ARCHIVE</p><DialogHeader><DialogTitle>午夜乘客档案</DialogTitle><DialogDescription>{language==='zh'?`遇见过的乘客会录入档案；第一次把人送到站，就能读到他的故事。最高抵达 ${highest}F · 故事 ${storiesUnlocked.length}/${Object.keys(STORIES).length}。`:`Riders you meet join the archive; deliver someone once to read their story. Best floor ${highest}F · Stories ${storiesUnlocked.length}/${Object.keys(STORIES).length}.`}</DialogDescription></DialogHeader><div className="archive-grid">{PASSENGER_ORDER.map((kind) => { const open = discovered.includes(kind); const spec = PASSENGERS[kind]; return <div className={`archive-item ${open ? '' : 'locked'}`} key={kind}>{open ? <Portrait kind={kind} /> : <LockKeyhole />}<span><b>{open ? spec.name : '？？？'}</b>{open&&<small>{spec.short}</small>}{storiesUnlocked.includes(kind)&&<details className="archive-story" data-no-translate><summary>{language==='zh'?'读故事':'Read story'}</summary><p>{STORIES[kind][language==='zh'?0:1]}</p></details>}</span></div>; })}</div>
    ...
    <Dialog open={run.status === 'upgrade' && doors === 'open'}><DialogContent className={`story-dialog upgrade-dialog ${upgradeCrisis ? 'upgrade-crisis' : ''}`} showCloseButton={false}>
      ...
      {upgradeCrisis && <p className="shop-warning" data-no-translate>{(()=>{const zh=language==='zh',plan=emergencyRepairPlan(run);
        ...
        const held=plan.affordable?(zh?` 抢救要 ${plan.cost} 币，这笔钱先留着：能力、配电箱、道具都不能花掉它。`:` The rescue costs ${plan.cost} coins and that money is held back: abilities, the power box and items cannot spend it.`):'';
        return `${base}${held}${sell}${margin}${zh?' 若无力修复，本班将在这里结束。':' If you cannot repair the elevator, the shift ends here.'}`;})()}</p>}
      <div className="shop-step-head" data-no-translate><h3><i>1</i>{language==='zh'?(run.shopUpgradeBought?`加购 1 项 · ${SHOP_PRICES.extraAbility} 金币（可跳过）`:'选 1 项能力 · 免费'):(run.shopUpgradeBought?`Buy 1 more · ${SHOP_PRICES.extraAbility} coins (optional)`:'Pick 1 ability · free')}</h3><span>
        {!run.shopUpgradeBought&&availableShopCards(run).length>0&&run.rerolledFloor!==run.floor&&<button className="shop-reroll" disabled={run.coins<REROLL_PRICE||spendLocked(REROLL_PRICE)} title={spendLocked(REROLL_PRICE)?rescueLockNote:undefined} onClick={()=>{const next=rerollShop(run, rngOf('shop', run.floor));if(next!==run){reportMetrics(run,next,language==='zh'?'重抽能力':'Reroll abilities');setRun(next);playTone(sound,'select');}}}><RotateCcw aria-hidden="true" />{language==='zh'?`重抽 · ${REROLL_PRICE}币`:`Reroll · ${REROLL_PRICE}c`}</button>}
</span></div>
      ...
      {(run.itemStock?.length??0)>0&&<section className="recharge-panel item-shop" data-no-translate><div className="shop-step-head"><h3><i>{run.stress>0||run.calmCharge?5:4}</i>{language==='zh'?'道具 · 一次性':'Items · single use'}</h3><span>{language==='zh'?`道具栏 ${run.items?.length??0}/${ITEM_SLOTS} · 同一种每买一次涨价`:`Bag ${run.items?.length??0}/${ITEM_SLOTS} · each repeat costs more`}</span></div>
        <div className="item-stock">{run.itemStock!.map((card,i)=>{const full=(run.items?.length??0)>=ITEM_SLOTS;return <button key={card.key} className={`item-card ${card.sold?'is-sold':''}`} disabled={card.sold||full||run.coins<card.price||spendLocked(card.price)} title={spendLocked(card.price)?rescueLockNote:undefined} onClick={()=>{const next=buyItem(run,i);if(next!==run){reportMetrics(run,next,language==='zh'?`买下${ITEMS[card.key].name}`:`Bought ${ITEMS[card.key].en}`);setRun(next);playSfx(sound,'register');}}}><span className="shop-icon item-icon" style={{backgroundImage:`url(${itemArt(card.key)})`}} aria-hidden="true" /><span className="item-card-text"><b>{language==='zh'?ITEMS[card.key].name:ITEMS[card.key].en}</b><small>{language==='zh'?ITEMS[card.key].zh:ITEMS[card.key].enText}</small></span><span className="item-price">{card.sold?(language==='zh'?'已买':'Bought'):full?(language==='zh'?'道具栏满':'Bag full'):language==='zh'?`${card.price} 币`:`${card.price}c`}</span></button>;})}</div>
      </section>}
  ...
  return <SymbolCodexContext.Provider value={setCodex}>{localizeTree(content, language)}</SymbolCodexContext.Provider>;
}
