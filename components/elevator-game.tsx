...
const DARK_LEGEND_REWARD: Record<DarkLegendKind, [string, string]> = {
  ...
  highroller: [`不是高躁动 +${DARK_LEGEND_RULES.highRollerWin} 金币，高躁动 −${DARK_LEGEND_RULES.highRollerLoss}`, `+${DARK_LEGEND_RULES.highRollerWin} coins unless agitation is high, then −${DARK_LEGEND_RULES.highRollerLoss}`],
...
function riderState(cabin: Array<Rider | null>, slot: number, bonus: number, agitation: number, floor = 0): { label: string; tone: 'active' | 'warn' | 'neutral' } | null {
  ...
  switch (rider.kind) {
    ...
    case 'highroller': return agitationBand(agitation) !== 'high' ? { label: `押注中 · 不是高躁动到站 +${DARK_LEGEND_RULES.highRollerWin}币`, tone: 'active' } : { label: `要输了 · 现在到站 −${DARK_LEGEND_RULES.highRollerLoss}币`, tone: 'warn' };
...
const MANUAL: Array<[string, string, string, string]> = [
  ...
  ['午夜与深渊', '离开 60 层商店后午夜钟声响起，越来越多的乘客以暗黑版出现，规则和链接都变了。80 层起是深渊：每 5 层加一级，暗黑版越来越极端——车费更高，但每层有几率发作（+3 躁动或吸电，卡上写着几率）。关门前会显示这一层的失控几率。', 'Midnight and the abyss', 'After the 60F shop the midnight bell rings and more and more riders arrive as their dark versions, with new rules and links. From 80F the abyss deepens every 5 floors: dark riders grow extreme—they pay more but may lash out each floor (+3 agitation or a power drain; the odds are on their cards). Before the doors close you see the chance this floor ends the run.'],
