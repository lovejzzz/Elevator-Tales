...
import { SYMBOLS, SYMBOL_GLYPH, SYMBOL_KEYS, greenBySymbol, symbolEdges, symbolEffectText, symbolShapes, symbolTitle, symbolsOf, type SymbolKey } from '@/lib/symbols';
...
function SymbolBadges({ rider, cabin, slot, locale }: { rider: Rider; cabin: Array<Rider | null>; slot: number; locale: GameLocale }) {
  ...
  return <span className="sym-badges">{syms.map((s: SymbolKey) => <span key={s} className={`sym sym-${s}`} title={symbolTitle(s, zh)} aria-label={zh ? SYMBOLS[s].zh : SYMBOLS[s].en}>{SYMBOL_GLYPH[s]}</span>)}</span>;
}
...
function upgradeImpact(key: UpgradeKey, run: RunState): string {
  ...
  switch (key) {
    case 'battery': return `🎉🎲 每级绿线每层 +${cooperationBonus(run) - 1} → +${cooperationBonus(preview) - 1} 金币。`;
...
const MANUAL: Array<[string, string, string, string]> = [
  ...
  ['符号与链接', '每位乘客有两个符号。邻座有相同符号＝绿线；有相反符号（🎉热闹↔🤫安静、📋秩序↔🎲江湖、🏠人间↔👻幽冥）＝红线；同一对邻座的相同和相反符号一对一抵消。车里每种符号只要有一条绿线，每层就生效一次：🎉 +2 币，🎲 +3 币但 +1 躁动，📋 和 🏠 −1 躁动，🤫 和 👻 省 1 电。同一符号连成一排再加 1 级、四人方块加 2 级、满车加 4 级。每条红线每层 +1 躁动。午夜后的暗黑版多是热闹、江湖、幽冥，和白天的乘客容易起冲突。', 'Symbols and links', 'Every rider carries two symbols. Neighbours who share a symbol = green link; neighbours with opposite symbols (🎉 Lively↔🤫 Quiet, 📋 Order↔🎲 Street, 🏠 Hearth↔👻 Spirit) = red link; a pair’s matches and clashes cancel one for one. Each symbol with at least one green link works once a floor: 🎉 +2 coins, 🎲 +3 coins but +1 agitation, 📋 and 🏠 −1 agitation, 🤫 and 👻 save 1 power. A row of one symbol adds a level, a 2×2 square two, a full cabin four. Each red link adds 1 agitation a floor. After midnight the dark riders are mostly Lively, Street and Spirit, and clash easily with daytime riders.'],
  ['三个数字', '每位乘客只看车费、每层耗电和躁动。每层耗电＝电梯运转＋乘客耗电−节能（🤫 和 👻 的绿线也省电）。', 'Three numbers', 'Every rider has a fare, power per floor and agitation. Power per floor = motor + riders − savings (🤫 and 👻 green links save power too).'],
...
const PRESSURE_RISE: Array<[string, string, string, string]> = [
  ...
  ['🔥 红线', '相反符号的邻座（🎉↔🤫、📋↔🎲、🏠↔👻）每条红线每层 +1 躁动；🎲 江湖的绿线也 +1。', '🔥 Red links', 'Neighbours with opposite symbols (🎉↔🤫, 📋↔🎲, 🏠↔👻): +1 agitation per red link per floor; 🎲 Street green links add 1 too.'],
