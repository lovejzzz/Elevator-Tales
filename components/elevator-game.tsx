...
export default function ElevatorGame() {
  ...
  const energyFatal = run.energy + energyPreview.highDelta <= 0;
    ...
    <section className="game-grid">
      <aside className="status-rail">
        ...
        <div data-metric="stress" className={`meter-card pressure ${agitated || pressurePreview.tone === 'danger' ? 'meter-danger' : ''}`} title={pressurePreview.summary}>
          ...
          <span className="mobile-meter-cap">上限 {run.stressCap}</span><span className="mobile-agitation-state">再 +{Math.max(0,run.stressCap-run.stress)} 失控</span><small className="rail-forecast"><span>下一站 <b className={stressFatal ? 'forecast-fatal' : ''}>{pressurePreview.range}</b></span><span>上限 {run.stressCap}</span></small><div className="agitation-state"><b>只计算人物</b><span>每位到站：本层最多 −1</span></div>
        </div>
