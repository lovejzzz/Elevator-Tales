...
export function PassengerCardFace({ rider, run, action, locale }: { rider: Rider; run: RunState; action?: string; locale: GameLocale }) {
  ...
  return <span className="unified-passenger-summary compact-card" data-no-translate>
    ...
    {legend ? <span className="cc-values cc-legend-values"><span>{rider.kind==='coldmatron'?(zh?'不付车费':'No fare'):(zh?'不付车费 · 不耗电':'No fare · no power')}</span></span> : <span className="cc-values">
      ...
      {/* v10.1: no net-value chips — the fare, power and agitation numbers and the links panel say it (players read −2 as “don’t board”). */}
    </span>}
  ...
  const content = <main className={`game-shell ${cooperationRelief(run) ? 'has-contract' : ''} ${difficultyTier(run.floor) % 2 ? 'phase-dawn' : ''}`}>
    ...
    <section className="game-grid">
      <aside className="status-rail">
        ...
        {run.status==='playing'&&run.marketFloor===run.floor&&run.marketStock&&<div className="night-market" data-no-translate><span className="item-bag-label">{language==='zh'?`夜市 · 道具 ${run.items?.length??0}/${ITEM_SLOTS}`:`Night market · items ${run.items?.length??0}/${ITEM_SLOTS}`}</span><div className="night-market-stall">{run.marketStock.map((card,i)=><button key={`${card.key}-${i}`} className={ITEMS[card.key].market?'is-night':undefined} disabled={locked||card.sold||run.coins<card.price||(run.items?.length??0)>=ITEM_SLOTS} onClick={()=>{const next=buyMarketItem(run,i);if(next!==run){reportMetrics(run,next,language==='zh'?'夜市':'Night market');setRun(next);playTone(sound,'upgrade');}}}><span className={`item-icon ${ITEMS[card.key].market?'is-night':''}`} style={{backgroundImage:`url(${itemArt(card.key)})`}} aria-hidden="true" /><span className="night-market-text"><span className="item-name">{language==='zh'?ITEMS[card.key].name:ITEMS[card.key].en}{ITEMS[card.key].market&&<em>{language==='zh'?'夜市货':'Night goods'}</em>}</span><small>{language==='zh'?ITEMS[card.key].zh:ITEMS[card.key].enText}</small></span><b>{card.sold?(language==='zh'?'已买':'Sold'):`${card.price}${language==='zh'?' 币':'c'}`}</b></button>)}</div></div>}
