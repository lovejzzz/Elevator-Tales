...
- Legend cards: "Deliver to 10F for keepsake · name: effect". Vinyl Record: "Musicians appear early (no need to wait for floor 16)" instead of "from floor 1". No rule changes.
...
- Boarding: rider settles in and the seat's gold frame closes and locks (no ring, sparks or late fly-in). Seats keep rarity: silver / gold + holo shader / double gold + glow + polychrome, with gem. No rule changes.
...
- Motion (motion 13.4.2) springs for offer cards, hover/press, seat settling, fly-ins and pops. WebGL foil for rare (holo with etched lines) and legendary (polychrome, rays, stars) cards via one shared context; CSS foil fallback; foil under text. Hover brightness steady. No rule changes.
...
- Seven district palettes (cabin tint, lamp glow, floor-number colour) cross-fading at each district boundary.
- Motion: ascent shake scaled by agitation (0.6–3.2 px), low-power flicker (≤6 worst-case power; faster when fatal), arrival coins flying to the wallet (2–6 per rider), rolling floor number, green links drawing in, breathing riders. All off under reduced motion.
- Shop icons render as background images (fixes 3 lint errors from v9.0.3). No rule changes.
- Art direction candidates (Art Deco poster vs noir ink vs current realism) generated for comparison; portraits unchanged pending a choice.
